    ...
    bolero::check!()
        ...
        .for_each(|(overlay_spec, probe_specs)| {
            ...
            for probe_spec in probe_specs {
                ...
                // The drawn summary is the built one now. `packet_for` used to clamp port 0 to
                // 1, since 0 is not a port either transport can carry, so the oracle had to be
                // asked about the clamped ports rather than the drawn ones. A probe's ports are
                // `NonZero`, so there is nothing left to clamp and nothing to restate.
                let judged = summary.clone();
