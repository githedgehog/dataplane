...
impl<M: Metadata + Clone> DpdkAclClassifier<M> {
    ...
    pub fn compile_for_socket(
        ...
        let comp = compiler::compile_categories(table).ok_or(CompileError::EmptyTable)?;
        ...
        let mut builder =
            AclClassifierBuilder::new("acl_classifier", socket_id, num_rules as u32, num_fields)?;
    ...
    pub fn classify<'a>(&'a self, headers: &Headers) -> acl::ClassifyOutcome<'a> {
        ...
        let best = compiler::resolve_categories(&self.table, &results, self.num_categories);
