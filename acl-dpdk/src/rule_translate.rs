...
#[must_use]
pub fn translate_rule<M: Metadata>(signature: FieldSignature, rule: &AclRule<M>) -> Vec<AclField> {
    ...
    if signature.has_ipv4_src() {
        fields.push(translate_ipv4_prefix(pm.ipv4().map(|m| &m.src)));
    }
    if signature.has_ipv4_dst() {
        fields.push(translate_ipv4_prefix(pm.ipv4().map(|m| &m.dst)));
    }
    // 8-byte fields
    ...
    if signature.has_icmp4_type() {
        fields.push(translate_icmp_byte(pm.icmp4().map(|m| &m.icmp_type)));
        two_byte_idx += 1;
    }
    if signature.has_icmp4_code() {
        fields.push(translate_icmp_byte(pm.icmp4().map(|m| &m.icmp_code)));
        two_byte_idx += 1;
    }
...
fn translate_ipv4_prefix(field: Option<&FieldMatch<acl::Ipv4Prefix>>) -> AclField {
    match field.and_then(FieldMatch::as_select) {
        Some(pfx) => AclField::from_u32(u32::from(pfx.addr()), u32::from(pfx.prefix_len())),
        None => AclField::wildcard(),
    }
}
...
fn translate_ipv6_prefix(field: Option<&FieldMatch<acl::Ipv6Prefix>>) -> (AclField, AclField) {
    match field.and_then(FieldMatch::as_select) {
        Some(pfx) => {
