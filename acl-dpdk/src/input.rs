...
pub fn count_two_byte_fields(sig: FieldSignature) -> usize {
    ...
    if sig.has_tcp_src() || sig.has_udp_src() {
        count += 1;
    }
    if sig.has_tcp_dst() || sig.has_udp_dst() {
        count += 1;
    }
    if sig.has_icmp4_type() {
        count += 1;
    }
    if sig.has_icmp4_code() {
        count += 1;
    }
    count
}
...
mod tests {
    ...
    fn compact_buffer_ipv4_only_promoted() {
        ...
        let rule = AclRuleBuilder::new()
            ...
            .ipv4(|ip| {
                ip.src =
                    FieldMatch::Select(Ipv4Prefix::new(Ipv4Addr::new(172, 16, 0, 0), 12).unwrap());
            })
            .permit(pri(100));

        let sig = acl::FieldSignature::from_match_fields(rule.packet_match());
        assert_eq!(
            count_two_byte_fields(sig),
            1,
            "should have 1 two-byte field (eth_type)"
        );
