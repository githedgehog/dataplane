...
fn build_context(table: &acl::AclTable) -> Option<(AclContext<4, Built>, acl::FieldSignature)> {
    ...
    let params = AclCreateParams::new::<N>("boundary_test", SocketId::ANY, max_rules as u32)
...
fn assert_agree(table: &acl::AclTable, packets: &[Headers]) {
    ...
    let Some((dpdk_ctx, sig)) = build_context(table) else {
        for pkt in packets {
            assert_eq!(classifier.classify(pkt, &()).fate(), table.default_fate(),);
        }
        return;
    };
    ...
    for pkt in packets {
        ...
        dpdk_ctx.classify(&data, &mut results, 1).expect("classify");
    ...
    let table = AclTableBuilder::new(Fate::Drop)
        .add_rule(
            AclRuleBuilder::new()
                .eth(|_| {})
                .ipv4(|ip| {
                    ip.src = FieldMatch::Select(Ipv4Prefix::any());
                })
                .tcp(|tcp| {
    ...
    let table = AclTableBuilder::new(Fate::Drop)
        .add_rule(
            AclRuleBuilder::new()
                ...
                .tcp(|tcp| {
                    tcp.dst = FieldMatch::Select(PortRange { min: 1, max: 65535 });
                })
                .permit(pri(100)),
        )
        .build();
    ...
    let table = AclTableBuilder::new(Fate::Drop)
        .add_rule(
            AclRuleBuilder::new()
                ...
                .tcp(|tcp| {
                    tcp.dst = FieldMatch::Select(PortRange { min: 0, max: 65535 });
                })
                .permit(pri(100)),
        )
        .build();
    ...
    let table = AclTableBuilder::new(Fate::Drop)
        .add_rule(
            AclRuleBuilder::new()
                ...
                .tcp(|tcp| {
                    tcp.dst = FieldMatch::Select(PortRange::exact(0u16));
                })
                .permit(pri(100)),
        )
        .build();
    ...
    for i in 0..50u32 {
        ...
        builder.push_rule(
            AclRuleBuilder::new()
                ...
                .ipv4(|ip| {
                    ip.src =
                        FieldMatch::Select(Ipv4Prefix::host(Ipv4Addr::new(10, 0, third, fourth)));
                })
                .tcp(|tcp| {
                    tcp.dst = FieldMatch::Select(PortRange::exact(80u16));
                })
                .action(
                    ActionSequence::just(if i % 2 == 0 { Fate::Accept } else { Fate::Drop }),
                    pri(i + 1),
                ),
        );
    }
    ...
    for i in 0..200u32 {
        ...
        builder.push_rule(
            AclRuleBuilder::new()
                .eth(|_| {})
                .ipv4(|ip| {
                    ip.src = FieldMatch::Select(Ipv4Prefix::host(Ipv4Addr::new(10, b3, b2, 1)));
                })
                .tcp(|tcp| {
                    tcp.dst = FieldMatch::Select(PortRange::exact(80u16));
                })
                .action(
                    ActionSequence::just(if i % 3 == 0 { Fate::Accept } else { Fate::Drop }),
                    pri(i + 1),
                ),
        );
    }
...
fn empty_table_default_fate() {
    ...
    assert!(
        groups.is_empty(),
        "empty table should produce no DPDK groups"
    );
