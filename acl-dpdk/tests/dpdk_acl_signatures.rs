...
fn classify_with_group(group: &CompiledGroup, packet: &Headers) -> u32 {
...
fn classify_n<const N: usize>(group: &CompiledGroup, data: &[*const u8], results: &mut [u32]) {
    let max_rules = group.rules().len().max(1);
    let params = AclCreateParams::new::<N>("sig_test", SocketId::ANY, max_rules as u32)
...
fn classify_all_groups(table: &acl::AclTable, groups: &[CompiledGroup], packet: &Headers) -> Fate {
...
fn two_signature_groups_ipv4_tcp_vs_ipv4_only() {
    ...
    for (i, g) in groups.iter().enumerate() {
        ...
        for (j, fd) in g.field_defs().iter().enumerate() {
            eprintln!(
                "  FieldDef[{j}]: type={:?} size={:?} fi={} ii={} offset={}",
    ...
    let udp_pkt = HeaderStack::new()
        .eth(|_| {})
        .ipv4(|ip| {
            ip.set_source(net::ipv4::UnicastIpv4Addr::new(Ipv4Addr::new(172, 16, 1, 1)).unwrap());
        })
        .udp(|udp| {
    ...
    let table = AclTableBuilder::new(Fate::Drop)
        .add_rule(
            AclRuleBuilder::new()
                ...
                .ipv4(|_| {})
                .tcp(|tcp| {
                    tcp.src = FieldMatch::Select(PortRange::exact(12345u16));
                    tcp.dst = FieldMatch::Select(PortRange::exact(80u16));
                })
                .permit(pri(100)),
        )
...
fn three_groups_mixed() {
    ...
    for (pkt, expected_fate, label) in &test_cases {
        ...
        assert_eq!(linear_fate, *expected_fate, "linear mismatch for {label}");
        assert_eq!(
...
fn classify_cat_n<const N: usize>(
    ...
    let max_rules = group.rules().len().max(1);
    let params = AclCreateParams::new::<N>("cat_test", SocketId::ANY, max_rules as u32)
        .expect("create params");
    let mut ctx = AclContext::<N>::new(params).expect("create context");
    ...
    let build_cfg =
        AclBuildConfig::new(categories, field_defs, 0).expect("build config with categories");
    let ctx = ctx.build(&build_cfg).expect("build context");
...
fn categories_three_groups_priority_across_groups() {
    ...
    assert_eq!(classifier.classify(&udp_pkt, &()).fate(), Fate::Accept,);
    assert_eq!(
