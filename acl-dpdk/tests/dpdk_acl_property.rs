...
fn random_port_range(a: u16, b: u16) -> PortRange<u16> {
...
fn build_table(specs: &[TestRuleSpec], default_fate: Fate) -> acl::AclTable {
    ...
    for (i, spec) in specs.iter().enumerate() {
        ...
        let rule = AclRuleBuilder::new()
            ...
            .tcp(|tcp| {
                tcp.dst = FieldMatch::Select(spec.dst_ports);
            })
            .action(
...
fn build_dpdk_context(
    ...
    let params = AclCreateParams::new::<N>("prop_test", SocketId::ANY, max_rules as u32)
...
fn assert_dpdk_matches_linear(table: &acl::AclTable, packets: &[Headers]) {
    ...
    let specs: Vec<TestRuleSpec> = (0..20u8)
        .map(|i| {
            ...
            TestRuleSpec {
                ...
                dst_ports: random_port_range(u16::from(i) * 100 + 1, u16::from(i) * 100 + 200),
                fate: if i % 3 == 0 { Fate::Drop } else { Fate::Accept },
            }
        })
        .collect();
    ...
    let table = build_table(
        &[
            ...
            TestRuleSpec {
                ...
                dst_ports: PortRange { min: 1, max: 65535 },
                fate: Fate::Drop,
            },
        ],
        Fate::Accept,
