...
#[derive(Default)]
pub struct IoCache(VecDeque<IoChunk>);
...
impl IoCache {
    ...
    /// drop all chunks from the cache not intended for `addr`
    pub fn clear_peer(&mut self, addr: &SocketAddr) {
        self.0
            .retain(|m| m.addr.as_pathname() != addr.as_pathname());
    }
}
