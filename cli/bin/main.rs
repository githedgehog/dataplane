...
///
/// Returns `None` rather than an error: the reason has already been printed to the terminal by
/// the time this returns, so there is nothing left for a caller to report.
