...
impl CliArgs {
    ...
    pub fn from_args_map(mut args_map: HashMap<String, String>) -> Result<CliArgs, ArgsError> {
        ...
        if let Some(addr) = take_arg(&mut args_map, "address")? {
            args.remote.address =
                Some(IpAddr::from_str(&addr).map_err(|_| ArgsError::BadAddress(addr))?);
        }
        ...
        if let Some(vrfid) = take_arg(&mut args_map, "vrfid")? {
            args.remote.vrfid = Some(vrfid.parse().map_err(|_| ArgsError::BadValue(vrfid))?);
        }
...
mod tests {
    ...
    fn valid_bind_address() {
        let args = CliArgs::from_args_map(map_from(&[("bind-address", "/tmp/cli.sock")])).unwrap();
    ...
    fn empty_address_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("address")),
            "got: {err}"
        );
    }
    ...
    fn empty_prefix_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("prefix")),
            "got: {err}"
        );
    }
    ...
    fn empty_bind_address_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("bind-address")),
            "got: {err}"
        );
    }
    ...
    fn empty_vrfid_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("vrfid")),
            "got: {err}"
        );
    }
    ...
    fn empty_ifname_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("ifname")),
            "got: {err}"
        );
    }
    ...
    fn empty_level_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("level")),
            "got: {err}"
        );
    }
    ...
    fn empty_protocol_is_missing_value() {
        ...
        assert!(
            matches!(err, ArgsError::MissingValue("protocol")),
            "got: {err}"
        );
    }
    ...
    fn mix_of_valid_and_unrecognized_is_rejected() {
        let err = CliArgs::from_args_map(map_from(&[("address", "10.0.0.1"), ("bogus", "value")]))
            .unwrap_err();
        assert!(
            matches!(err, ArgsError::UnrecognizedArgs(ref m) if m.contains_key("bogus")),
            "got: {err}"
        );
    }
