...
The same principle applies when a `Subscriber` drops. By default
rust drops fields in declared order, which would drop `cached` after
`epoch` — wrong direction. The crate's `Drop` impl explicitly clears
`cached` first:
...
sequenceDiagram
    ...
    loop for each cell in active
        ...
        alt yes (Subscriber gone)
            ...
            D->>Cs: load() [Acquire]
            Cs-->>D: observed
            alt observed == 0 (registered, never snapshotted)
                D->>D: skip from min computation
            else observed > 0
                D->>D: track if min > observed
            end
        end
    end
    D-->>P: Option[Version]
    ...
    else None (no live Subscribers, or none have snapshotted)
        P->>P: retired.clear()
    end
```
...
- **`observed == 0`** means a registered `Subscriber` that hasn't yet
  taken a snapshot. That `Subscriber` holds no `Versioned` clone, so it
  pins nothing; we skip it for the min computation but keep its slot.
  This is why `Epoch::new()` initializes the cell to `0`: a registered-
  but-not-yet-observed Subscriber can't accidentally pin the initial
  version forever.
...
- **Single publisher.** The `!Sync` Publisher and `&self` publish/
  ...
  different design.
- **Reclaim is opportunistic.** `Publisher::publish` runs a reclaim
  pass automatically; for amortization you can call `Publisher::reclaim`
  explicitly between publishes. There is no way to _force_ reclamation
  past a stuck Subscriber other than dropping that Subscriber.
