...
/// Shared pipeline generation state.
///
/// `genid` is the published generation used to reject stale flows. `staging` stamps newly
/// created or revalidated flows. Open the staging generation before migration and publish it
/// after installing the configuration. This keeps flows created during the update from becoming
/// stale when it completes.
#[derive(Default, Debug)]
pub struct PipelineData {
    /// Published generation used to reject stale flows.
    genid: AtomicI64,
    /// Generation stamped onto new or revalidated flows.
    staging: AtomicI64,
}
impl PipelineData {
    #[must_use]
    ...
    /// Read the generation for newly validated flows.
    ...
    /// Publish `genid` and set it as the stamping generation.
