...
impl CmdArgs {
    ...
    /// Get the metrics port (for backward compatibility), returns None if metrics are disabled
    pub fn metrics_port(&self) -> Option<u16> {
        if self.disable_metrics {
            None
        } else {
            // Extract port from the address for backward compatibility
            match self.parse_metrics_address() {
