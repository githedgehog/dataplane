...
impl DriverKernel {
    ...
    pub fn compute_worker_idx(pkt: &Packet<TestBuffer>, workers: usize) -> usize {
        let n = workers.max(1);
        // Try flow-key-based symmetric hash (A<->B map to same bucket)
    ...
    fn spawn_workers(
        ...
        Vec<chan::Sender<Packet<TestBuffer>>>,
        chan::Receiver<Packet<TestBuffer>>,
    )> {
        let w = workers.max(1);
        ...
        for wid in 0..w {
            ...
            let handle_res = builder.spawn(move || {
                ...
                loop {
                    match rx_to_worker.recv() {
                        Ok(pkt) => {
                            // Log packet with thread and worker ID
                            tracing::debug!(
                                worker = wid,
                            ...
                            for out_pkt in pipeline.process(std::iter::once(pkt)) {
                                ...
                                if tx_out.send(out_pkt).is_err() {
                                    // dispatcher gone; exit
                                    break;
                                }
                            }
                        }
                        Err(_) => break, // dispatcher dropped the channel; exit
                    }
                }
            });
            ...
            if let Err(e) = handle_res {
                ...
                return Err(io::Error::new(io::ErrorKind::Other, "worker spawn failed"));
            }
        }
    ...
    pub fn start(
        ...
        loop {
            ...
            while let Ok(mut pkt) = from_workers.try_recv() {
                ...
                let oif_id_opt = pkt.get_meta().oif.as_ref().map(|id| id.get_id());
                if let Some(oif_id) = oif_id_opt {
                    ...
                    // No oif set; nothing to send (likely dropped upstream).
                    debug!("No oif in packet meta; dropping");
                }
            }
