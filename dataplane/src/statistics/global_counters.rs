...
use metrics::{gauge, describe_gauge, counter, describe_counter};
...
pub fn init_metrics() {
    ...
    describe_counter!(METRICS_REQUESTS_TOTAL, "Total number of /metrics endpoint requests");
}
...
pub fn sync_to_prometheus(vpc_counters: Vec<VpcCounters>) {
    ...
    for vpc in vpc_counters {
        ...
        // Drops
        gauge!(VPC_DROPS_TOTAL, "vpc" => vpc_name)
            .set(vpc.drops as f64);
    }
}
