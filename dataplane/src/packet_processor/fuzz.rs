...
mod generated {
    ...
    static EXCEPTING: AtomicU64 = AtomicU64::new(0);

    static AGED: AtomicU64 = AtomicU64::new(0);
    static AGED_MILLIS: AtomicU64 = AtomicU64::new(0);
    ...
    fn a_configuration_carries_nothing_it_denies() {
        ...
        static BY_ACL: AtomicU64 = AtomicU64::new(0);
    ...
    fn an_excluded_address_is_not_reachable() {
        static AIMED: AtomicU64 = AtomicU64::new(0);
        static REFUSED: AtomicU64 = AtomicU64::new(0);
        let _eal = dpdk::test_support::start_eal();
...
mod model {
    ...
    fn two_workers_are_not_given_the_same_public_tuple() {
        ...
        static BARE: AtomicU64 = AtomicU64::new(0);
        static UNBUILT: AtomicU64 = AtomicU64::new(0);
                    ...
                    for (code, tears_down) in [
                        ...
                        (
                            net::icmp4::Icmp4DestUnreachable::FragmentationNeeded {
                                next_hop_mtu: Some(
                                    1400.try_into().unwrap_or_else(|_| unreachable!()),
                                ),
                            },
                            false,
                        ),
                        ...
                        let (doomed, mut spared): ((IpAddr, u16), Conversation) =
                            thread::scope(|scope| {
                                let doomed = thread::Builder::new()
                                    .name("open-doomed".to_owned())
                                    .spawn_scoped(scope, move || {
                                        let _guard =
                                            opening.as_ref().map(tokio::runtime::Handle::enter);
                                        ...
                                        .unwrap_or_else(|| {
                                            unreachable!("the fixture request builds")
                                        });
                                ...
                                let spared = thread::Builder::new()
                                    ...
                                    .spawn_scoped(scope, move || {
                                        let _guard =
                                            answering.as_ref().map(tokio::runtime::Handle::enter);
                        ...
                        let spared = thread::scope(|scope| {
                            let teardown = thread::Builder::new()
                                ...
                                .spawn_scoped(scope, move || {
                                    let _guard =
                                        tearing.as_ref().map(tokio::runtime::Handle::enter);
                                    let _evidence = tracectl::evidence::capture("teardown");
                                    let mut worker = blueprint.worker();
                                    let out = worker.send(dst_unreachable(code, doomed, target));
                                    matches!(verdict(&out), Verdict::Delivered { .. })
                                })
                            ...
                            let answer = thread::Builder::new()
                                ...
                                .spawn_scoped(scope, move || {
                                    let _guard =
                                        keeping.as_ref().map(tokio::runtime::Handle::enter);
                        ...
                        let out =
                            worker.send(super::routed::tunnelled_from(vni(REMOTE_VNI), &reply));
                                ...
                                thread::Builder::new()
                                    ...
                                    .spawn_scoped(scope, move || {
                                        ...
                                        let probe = |worker: &mut Worker, port: &mut u16| -> Seen {
                                            without_unwinding(|| send(worker, port)).unwrap_or_else(
                                                |why| Err(format!("sending it panicked: {why}")),
                                            )
                                        };
