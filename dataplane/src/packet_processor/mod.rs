...
mod test {
    ...
    fn build_processor_pipeline() -> DynPipeline<TestBuffer> {
        let iftable = Arc::new(build_test_iftable());
        let stage_ingress = Ingress::new("Ingress", &iftable.clone());
        let stage_egress = Egress::new("Egress", &iftable.clone());
        ...
        let stats = PacketDropStats::new("Pipeline-stats");

        DynPipeline::with_name("Router")
    ...
    fn build_test_ipv4_pkt(dst_ip: &str, dst_mac: Mac, proto: NextHeader) -> Packet<TestBuffer> {
        ...
        ipv4.set_ttl(255);
        ipv4.set_next_header_unchecked(proto);
    ...
    fn build_test_packets() -> Vec<Packet<TestBuffer>> {
        ...
        let p1 = build_test_ipv4_pkt("10.0.0.2", Mac([0x2, 0, 0, 0, 0, 29]), NextHeader::TCP)
        ...
        let p5 = build_test_ipv4_pkt("11.0.0.4", Mac([0x2, 0, 0, 0, 0, 1]), NextHeader::ICMP)
    ...
    fn test_pktprocessor() {
        init_test_tracing();
        let mut pipeline = build_processor_pipeline();
        ...
        for pkt in packets_out.iter_mut() {
            match pkt.meta.descr {
                ...
                "forward" => {
                    assert!(!pkt.meta.is_l2bcast);
                    assert!(!pkt.meta.is_iplocal);
                    pkt.pkt_drop(DropReason::Delivered);
                }
                "filtered" => {
