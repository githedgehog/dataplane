...
fn route_packet<Buf: PacketBufferMut>(
    ...
    /* WARNING: The following is just a mock for testing the new pipeline format */
...
impl<Buf: PacketBufferMut> NetworkFunction<Buf> for IpForwarder {
    ...
    fn process<'a, Input: Iterator<Item = Packet<Buf>> + 'a>(
        ...
        input.filter_map(|mut packet| {
            if !packet.dropped() {
                if let Some(vrfid) = packet.meta.route {
