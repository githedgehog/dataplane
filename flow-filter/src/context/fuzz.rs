...
pub(crate) fn oracle_lookup(overlay: &ValidatedOverlay, probe: &Probe) -> LookupResult {
    ...
    // The oracle models the key encoding, sentinel and all: a probe with no ports lowers to the
    // same zero the real key does, and so matches exactly the wildcard ranges.
...
fn dpdk_backend_matches_reference_on_generated_overlays() {
    ...
    bolero::check!()
        ...
        .for_each(|(overlay_spec, probe_specs)| {
            ...
            let reference = FlowFilterContext::build(&built.overlay, Backend::Reference)
