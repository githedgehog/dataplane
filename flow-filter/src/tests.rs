...
/// A protocol-restricted expose matches TCP whether or not an extension header precedes it.
///
/// IPv6 extension headers occupy `Net::next_header()`, so reading that field as the protocol
/// classifies this packet as Hop-by-Hop and no TCP expose covers it. The same root cause is a
/// security hole one stage over -- in `acl-filter` it let an extension header carry a packet past
/// a Deny rule -- and a functional one here, where it dropped traffic somebody had configured a
/// path for. `Headers::upper_layer_proto` is what both now ask.
#[test]
