...
/// Build a flow-filter context (reference backend: fast, EAL-free) from a set of VPCs and peerings.
pub(crate) fn context(vpcs: &[(&str, u32)], peerings: Vec<VpcPeering>) -> FlowFilterContext {
    FlowFilterContext::for_test(&overlay(vpcs, peerings))
}
