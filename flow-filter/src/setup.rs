...
impl FlowFilterTable {
    ...
    fn add_peering(
        ...
        let (local_prefixes, remote_prefixes) =
            get_prefixes_for_processing(overlay, vpc, peering, dst_vpcd, true);

        self.no_ports.process_peering(
            local_vpcd,
            dst_vpcd,
            local_prefixes,
            remote_prefixes,
        ...
        .filter(|expose| !expose.is_default())
    {
            ...
            .filter(|expose| !expose.is_default())
        {
            ...
            for prefix_left in get_ips(expose_left).iter() {
                if skip_ports && prefix_left.ports().is_some() {
                    // Skip prefixes with ports
                    continue;
                }
                for prefix_right in get_ips(expose_right).iter() {
                    if skip_ports && prefix_right.ports().is_some() {
                        // Skip prefixes with ports
                        continue;
                    }
...
mod tests {
    ...
    fn test_get_manifest_ips_overlap_no_overlap() {
        ...
        let mut manifest1 = VpcManifest::with_exposes(
            "manifest1",
            vec![VpcExpose::empty().ip("10.0.0.0/24".into())],
        );
        manifest1.validate().unwrap();

        let mut manifest2 = VpcManifest::with_exposes(
            "manifest2",
    ...
    fn test_get_split_prefixes_for_manifest_no_overlap() {
        ...
        let mut manifest = VpcManifest::with_exposes(
            "manifest",
            vec![VpcExpose::empty().ip("10.0.0.0/24".into())],
        );
    ...
    fn test_get_split_prefixes_for_manifest_with_two_overlaps() {
        let vpcd_a = vpcd(100);
        let remote_data_a = RemoteData::new(vpcd_a, None, None);
        let remote_data_b = RemoteData::new(vpcd(200), None, None);
        ...
        overlaps.insert(
            PrefixWithOptionalPorts::new(Prefix::from("10.0.0.0/25"), None),
            HashSet::from([remote_data_a, remote_data_b]),
        );
        // Overlap 2: 10.0.0.128/25 is shared between A and C
        overlaps.insert(
            PrefixWithOptionalPorts::new(Prefix::from("10.0.0.128/25"), None),
            HashSet::from([remote_data_a, remote_data_c]),
        );
        ...
        assert_eq!(
            result[0].1,
            VpcdLookupResult::MultipleMatches(HashSet::from([remote_data_a, remote_data_b])),
        ...
        assert_eq!(
            result[1].1,
            VpcdLookupResult::MultipleMatches(HashSet::from([remote_data_a, remote_data_c])),
    ...
    fn test_process_peering_with_overlap() {
        ...
        // vpc1 is not valid, we have to fake its transformation into a Vpc for this test
        assert!(matches!(
            vpc1.clone().validate(),
            Err(ConfigError::OverlappingPrefixes(_, _))
        ));
