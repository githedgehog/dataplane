...
impl DevicesNotFound {
    pub(crate) fn new<'a>(
        ...
        Self {
            missing: missing.copied().collect(),
        }
    }
}
...
impl DeviceSearch {
    ...
    pub fn new<'a>(requested: impl Iterator<Item = &'a NetworkDeviceDescription>) -> DeviceSearch {
        let requested: BTreeSet<_> = requested.copied().collect();
    ...
    pub fn missing(&self) -> BTreeSet<NetworkDeviceDescription> {
        let matching: BTreeSet<_> = self.matching().keys().copied().collect();
        self.requested.difference(&matching).copied().collect()
    }
    ...
    pub fn scheduled_for_use(&self) -> BTreeMap<NetworkDeviceDescription, &hardware::Node> {
        ...
        let supportable_keys: BTreeSet<_> = supportable.keys().copied().collect();
    ...
    pub fn requested_but_not_supported(&self) -> BTreeMap<String, &hardware::Node> {
        ...
        let matching_keys: BTreeSet<_> = matching.keys().copied().collect();
        let supportable = self.supportable();
        let supportable_keys: BTreeSet<_> = supportable.keys().copied().collect();
...
impl<'search> Display for NetworkDeviceSearchReport<'search> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
