...
mod tests {
    ...
    // Proves an rte_acl key can carry proto as a #[mask] field (any = mask 0x00) plus one or two
    // #[exact] u32 VNI fields alongside prefix + range, for both IPv4 and IPv6, and that lookup /
    // lookup_batch classify correctly. proto is FIRST because rte_acl requires a 1-byte first field.
    ...
    #[test]
    #[dpdk::with_eal]
    fn flofi_two_table_key_shapes_build_and_classify() {
            ...
            vec![
                ...
                RuleSpec::<RemoteKey<Ipv4Addr>, u32>::new(
                    ...
                    RemoteKeyRule {
                        proto: any(),
                        src_vpcd: ExactSpec::new(100),
                        dst_ip: PrefixSpec::new(Ipv4Addr::new(0, 0, 0, 0), 0),
                        dst_port: anyport(),
                    }
                    .into_backend_fields::<Dpdk>(),
                    0xDD,
    ...
    // ---------------------------------------------------------------------------------------------
    // Characterization probe for the flofi two-table capacity gate (flofi/docs/two-table-redesign.md).
    // Finds where rte_acl build cost / memory explodes, to inform the config-layer rule-count limit.
    // Two rule shapes on the real RemoteKey: `disjoint` (distinct /24s, one VPC, any proto/port --
    // models a config-legal table) vs `overlapping` (shared /24, nested overlapping port ranges, one
    // VPC -- adversarial; the shape config MUST forbid). NOTE: runs against the debug DPDK sysroot,
    // so absolute ms are not production numbers -- the curve SHAPE and the OOM point are the signal.
    // Run explicitly:
    //   cargo test -p dataplane-acl --features dpdk,reference --lib flofi_build_capacity_probe -- --ignored --nocapture
    #[test]
    #[ignore = "characterization probe; run with --ignored --nocapture"]
    #[dpdk::with_eal]
    fn flofi_build_capacity_probe() {
        use crate::dpdk::lookup::DpdkAclLookup;
        use crate::dpdk::rule::{Dpdk, RuleSpec};
        use core::net::Ipv4Addr;
        use match_action::MaskSpec;
        ...
        println!("\n=== flofi RemoteKey<Ipv4Addr> build capacity (debug dpdk sysroot) ===");
