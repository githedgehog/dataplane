...
pub fn plan_layout(specs: &[FieldSpec]) -> Result<DpdkLayout, LayoutError> {
    ...
    let mut user_to_dpdk: ArrayVec<usize, MAX_FIELDS> = ArrayVec::new();
    let mut user_chunk_counts: ArrayVec<usize, MAX_FIELDS> = ArrayVec::new();
    for _ in 0..specs.len() {
        user_to_dpdk
            .try_push(0)
            .map_err(|_| LayoutError::TooManyFieldDefs { total: specs.len() })?;
        user_chunk_counts
            .try_push(0)
            .map_err(|_| LayoutError::TooManyFieldDefs { total: specs.len() })?;
    }
