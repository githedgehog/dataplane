...
fn vlan_matches(m: &VlanMatch, headers: &Headers) -> bool {
    ...
    m.vid.matches(|v| vlan.vid() == *v)
        ...
        && m.inner_ether_type.matches(|et| vlan.inner_ethtype() == *et)
}
...
fn ipv4_matches(m: &Ipv4Match, headers: &Headers) -> bool {
    ...
    m.src
        .matches(|pfx| ipv4_in_prefix(*pfx, ip.source().into()))
...
fn ipv6_matches(m: &Ipv6Match, headers: &Headers) -> bool {
    ...
    m.src
        .matches(|pfx| ipv6_in_prefix(*pfx, ip.source().into()))
...
fn tcp_matches(m: &TcpMatch, headers: &Headers) -> bool {
    ...
    m.src.matches(|r| port_in_range(*r, tcp.source().as_u16()))
        && m.dst
...
fn udp_matches(m: &UdpMatch, headers: &Headers) -> bool {
    ...
    m.src.matches(|r| port_in_range(*r, udp.source().as_u16()))
        && m.dst
...
mod tests {
    ...
    fn ipv4_prefix_match() {
        ...
        let matching_headers = HeaderStack::new()
            ...
            .unwrap();
        assert_eq!(
            classifier.classify(&matching_headers, &()).fate(),
            Fate::Accept
        );
        ...
        let non_matching_headers = HeaderStack::new()
            ...
            .unwrap();
        assert_eq!(
            classifier.classify(&non_matching_headers, &()).fate(),
            Fate::Drop
        );
        ...
        let leading_vlan_protocol = HeaderStack::new()
            ...
            .unwrap();
        assert_eq!(
            classifier.classify(&leading_vlan_protocol, &()).fate(),
            Fate::Drop
        );
        ...
        let priority_tagged = HeaderStack::new()
            ...
            .unwrap();
        assert_eq!(
            classifier.classify(&priority_tagged, &()).fate(),
            Fate::Drop
        );
    }
