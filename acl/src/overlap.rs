...
fn field_match_overlaps_prefix_v4(a: FieldMatch<Ipv4Prefix>, b: FieldMatch<Ipv4Prefix>) -> bool {
    match (a.as_select(), b.as_select()) {
        (None, _) | (_, None) => true,
...
fn field_match_overlaps_prefix_v6(a: &FieldMatch<Ipv6Prefix>, b: &FieldMatch<Ipv6Prefix>) -> bool {
    match (a.as_select(), b.as_select()) {
        (None, _) | (_, None) => true,
...
fn field_match_overlaps_range<T: Ord>(
    a: &FieldMatch<PortRange<T>>,
    b: &FieldMatch<PortRange<T>>,
) -> bool {
    match (a.as_select(), b.as_select()) {
        (None, _) | (_, None) => true,
        (Some(ra), Some(rb)) => ra.min <= rb.max && rb.min <= ra.max,
    }
}
...
fn prefixes_overlap_v4(a: Ipv4Prefix, b: Ipv4Prefix) -> bool {
    let shorter_mask = if a.prefix_len() <= b.prefix_len() {
        a.mask()
    } else {
        b.mask()
    };
    // If both network addresses, masked to the shorter prefix, are equal,
    // the prefixes overlap.
    (u32::from(a.addr()) & shorter_mask) == (u32::from(b.addr()) & shorter_mask)
}
...
fn prefixes_overlap_v6(a: Ipv6Prefix, b: Ipv6Prefix) -> bool {
    ...
    (u128::from(a.addr()) & shorter_mask) == (u128::from(b.addr()) & shorter_mask)
}
...
mod tests {
    ...
    fn disjoint_prefixes_do_not_overlap() {
        ...
        let r2 = AclRuleBuilder::new()
            ...
            .ipv4(|ip| {
                ip.src =
                    FieldMatch::Select(Ipv4Prefix::new(Ipv4Addr::new(192, 168, 0, 0), 16).unwrap());
            })
            .deny(pri(200));
    ...
    fn disjoint_port_ranges_do_not_overlap() {
        let r1 = AclRuleBuilder::new()
            ...
            .ipv4(|_| {})
            .tcp(|tcp| {
                tcp.dst = FieldMatch::Select(PortRange::new(80u16, 89u16).unwrap());
            })
            .permit(pri(100));
        ...
        let r2 = AclRuleBuilder::new()
            ...
            .ipv4(|_| {})
            .tcp(|tcp| {
                tcp.dst = FieldMatch::Select(PortRange::new(443u16, 449u16).unwrap());
            })
            .deny(pri(200));
    ...
    fn overlapping_port_ranges() {
        ...
        let r2 = AclRuleBuilder::new()
            ...
            .ipv4(|_| {})
            .tcp(|tcp| {
                tcp.dst = FieldMatch::Select(PortRange::new(200u16, 500u16).unwrap());
            })
            .deny(pri(200));
