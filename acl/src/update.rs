...
pub fn diff_tables<M: Metadata + Clone + PartialEq>(
    ...
    let new_by_priority: HashMap<Priority, &AclRule<M>> =
        new.rules().iter().map(|r| (r.priority(), r)).collect();
...
pub fn plan_update<M: Metadata + Clone + PartialEq>(
    ...
    if use_delta {
        ...
        let mut delta_builder = crate::table::AclTableBuilder::new(new_table.default_fate());
...
mod tests {
    ...
    fn two_tier_matches_fresh_compile() {
        ...
        for i in 1..=15u32 {
            builder.push_rule(
                AclRuleBuilder::new()
                    .eth(|_| {})
                    .ipv4(|ip| {
                        ip.src = FieldMatch::Select(Ipv4Prefix::host(Ipv4Addr::new(
                            10,
                            0,
                            0,
                            i.try_into().unwrap(),
        ...
        new_builder.push_rule(
            AclRuleBuilder::new()
                .eth(|_| {})
                .ipv4(|ip| {
                    ip.src = FieldMatch::Select(Ipv4Prefix::host(Ipv4Addr::new(192, 168, 1, 1)));
                })
                .deny(pri(100)),
        );
        ...
        assert!(
            plan.needs_merge,
            "should use delta strategy for 1 change in 16 rules"
        );
    ...
    fn removed_rule_reflected_in_delta() {
        ...
        assert_eq!(
            tiered_fate,
            Fate::Drop,
            "removed rule should not match in delta"
        );
