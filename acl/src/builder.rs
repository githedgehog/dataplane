...
mod tests {
    ...
    impl Metadata for TestMeta {
        ...
        fn matches_values(&self, values: &TestMetaValues) -> bool {
            self.vrf.as_ref().is_none_or(|m| *m.value() == values.vrf)
                && self.vni.as_ref().is_none_or(|m| *m.value() == values.vni)
        }
    }
    ...
    fn ipv6_udp_rule() {
        let rule = AclRuleBuilder::new()
            .eth(|_| {})
            .ipv6(|ip| {
                ip.dst = FieldMatch::Select(Ipv6Prefix::from(Ipv6Addr::LOCALHOST));
            })
            .udp(|udp| {
    ...
    fn metadata_before_layers() {
        ...
        assert_eq!(rule.metadata().vrf, Some(ExactMatch::new(42)));
    ...
    fn metadata_between_layers() {
        let rule = AclRuleBuilder::new()
            .eth(|_| {})
            .metadata(|m: &mut TestMeta| {
                m.vni = Some(ExactMatch::new(1000));
            })
            .ipv4(|_| {})
            .tcp(|_| {})
            .deny(pri(50));

        assert_eq!(rule.metadata().vni, Some(ExactMatch::new(1000)));
        let ipv4 = rule.packet_match().ipv4().unwrap();
        assert_eq!(ipv4.protocol, FieldMatch::Select(NextHeader::TCP));
    }
