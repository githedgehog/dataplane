...
impl<M: Metadata> ClassifierInner<M> {
    fn classify<'a>(&'a self, headers: &Headers, metadata: &M::Values) -> ClassifyOutcome<'a> {
        match self {
            Self::Linear {
                rules,
                default_fate,
...
impl<M: Metadata> Classifier<M> {
    ...
    /// default [`Fate`].
    #[must_use]
    pub fn classify<'a>(&'a self, headers: &Headers, metadata: &M::Values) -> ClassifyOutcome<'a> {
    ...
    fn priority_ordering_through_compile() {
        ...
        for (ip, expected) in test_ips {
            let headers = HeaderStack::new()
                .eth(|_| {})
                .ipv4(|ip_hdr| {
                    ip_hdr.set_source(net::ipv4::UnicastIpv4Addr::new(ip).unwrap());
                })
                .tcp(|_| {})
                .build_headers()
                .unwrap();
