...
//!
//! Both halves fire. Deleting one row of [`REACH`] fails
...
const REACH: &[(&str, Reach)] = &[
    ...
    (
        "VpcPeering.acl",
        // Presence only. What the algebra reaches *inside* an ACL is the twelve rows below, and
        // they are the ones worth reading: presence alone would make a vocabulary of one rule
        // shape look like a vocabulary of ACLs.
        Reach::Spans(&["absent", "present"]),
    ),
    (
        "Acl.default",
        // Both, because the two guards set it opposite to what their rules say -- which is what
        // makes the rules load-bearing in either direction. See `algebra::Guard`.
        Reach::Spans(&["allow", "deny"]),
    ),
    (
        "Acl.rules",
    ...
    (
        "AclRule.scope",
        // `Flow` only through `Guard::PermitFlow`, which carries `validate_scope`'s condition as
        // its own precondition -- see `algebra::Guard::legal_on`.
        Reach::Spans(&["flow", "packet"]),
    ),
    (
        "AclRule.log",
