...
/// Rotate and swap `items`, reporting whether the order actually changed.
///
/// The caller counts these to tell a run where the permutation did nothing from a
/// run where it did something and the configuration was unmoved by it.
///
/// The answer has to come from the resulting *order*, not from whether the two
/// operations were individually non-trivial: they compose, and they can compose to
/// the identity. Rotating a two-element list by one and then swapping 0 and 1 puts
/// it back exactly as it was, and the operation-counting version reported that as a
/// reordering. Every such case inflated the non-vacuity counter in
/// `a_configuration_has_only_one_meaning`, letting it clear its movement threshold
/// on cases that never exercised order-dependence at all.
///
/// Tracking a permutation of the indices rather than diffing `T` keeps this free of
/// a `PartialEq` bound, and is the honest comparison anyway: a list of equal
/// elements is genuinely reordered even though the values do not look different.
fn reorder<D: Driver, T>(d: &mut D, items: &mut [T]) -> Option<bool> {
    ...
    Some(order.iter().enumerate().any(|(to, from)| to != *from))
}
