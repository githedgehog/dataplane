...
fn distinguishing_bits(count: u16) -> u32 {
    ...
    // Widened to u32 before rounding up.  `u16::next_power_of_two` wraps to zero
    // for anything above 32768, and `ilog2` panics on zero, so the u16 form turned
    // a large count into a panic rather than a wide prefix.
