...
# Run tests with code coverage. Args are forwarded to nextest, so scope with e.g.
# `just coverage -p dataplane-nat`; with no args it covers the workspace.
#
# This builds its own instrumented binaries rather than replaying the nextest archive that
# `just test` uses. Coverage needs `-Cinstrument-coverage` at compile time, and the archive is
# built without it, so replaying it runs every test and writes no profile data at all -- the
# report step then fails with "not found *.profraw files", having looked like it worked right up
# until the end. Reusing the archive would mean teaching the nix build to produce an instrumented
# one, which is worth doing only if this ever moves into CI.
[script]
coverage *args:
    {{ _just_debuggable_ }}
