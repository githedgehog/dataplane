...
fn select_or_zero_mac(field: &FieldMatch<Mac>) -> Mac {
    field.as_select().copied().unwrap_or(Mac([0; 6]))
}
...
fn select_or_zero_ethtype(field: &FieldMatch<EthType>) -> EthType {
    field.as_select().copied().unwrap_or(EthType::from(0u16))
}
...
mod tests {
    ...
    fn compile_simple_drop_rule() {
        ...
        assert!(
            compiled
                .pattern
                .iter()
                .any(|m| matches!(m, FlowMatch::Eth(_)))
        );
        assert!(
            compiled
                .pattern
                .iter()
                .any(|m| matches!(m, FlowMatch::Ipv4(_)))
        );
        ...
        assert!(
            compiled
                .actions
                .iter()
                .any(|a| matches!(a, FlowAction::Drop))
        );
        assert!(
            compiled
                .actions
                .iter()
                .any(|a| matches!(a, FlowAction::End))
        );
    }
    ...
    fn compile_forward_rule() {
        ...
        assert!(
            compiled
                .pattern
                .iter()
                .any(|m| matches!(m, FlowMatch::Tcp(_)))
        );
        assert!(
            compiled
                .actions
    ...
    #[test]
    fn reject_tag_step() {
        let rule = AclRuleBuilder::new().eth(|_| {}).ipv4(|_| {}).action(
            acl::ActionSequence::new(vec![Step::Tag { index: 0, value: 1 }], Fate::Accept),
            pri(100),
        );
