            ...
            (Some(orig_prefix), Some(target_prefix), Some(orig_addr), Some(target_addr)) => {
                ...
                match prefix_orig_remain_size.partial_cmp(&prefix_target_remain_size) {
                    Some(Ordering::Less) => {
                        ...
                        let Ok(new_range_end) = add_offset_to_address(&orig_addr, range_size)
                        else {
                    ...
                    Some(Ordering::Equal) => {
                        ...
                        let Ok(new_range_end) =
                            add_offset_to_address(&orig_addr, prefix_orig_remain_size)
                        else {
                            return Some(Err(NatPeeringError::MalformedPeering));
                        };
                        value.orig_range_end = new_range_end;
...
pub fn add_peering(table: &mut PerVniTable, peering: &Peering) -> Result<(), NatPeeringError> {
    ...
    new_peering.local.exposes.iter().try_for_each(|expose| {
        ...
        generate_public_trie_values(expose).try_for_each(|value| {
            peering_table
                .insert(&value?)
    ...
    new_peering.remote.exposes.iter().try_for_each(|expose| {
        ...
        generate_private_trie_values(expose).try_for_each(|value| {
            table
                .dst_nat
                .insert(&value?)
                .map_err(|_| NatPeeringError::EntryExists)
