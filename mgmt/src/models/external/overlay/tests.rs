...
fn test_vpc_collect_peerings() {
    ...
    let mut vpc_table = MultiIndexVpcMap::default();
    vpc_table
        .try_insert(Vpc::new("VPC-1", "AAAAA", 3000).expect("Should succeed"))
        .unwrap();
    vpc_table
        .try_insert(Vpc::new("VPC-2", "BBBBB", 4000).expect("Should succeed"))
        .unwrap();
    vpc_table
        .try_insert(Vpc::new("VPC-3", "CCCCC", 2000).expect("Should succeed"))
        .unwrap();
    vpc_table
        .try_insert(Vpc::new("VPC-4", "DDDDD", 6000).expect("Should succeed"))
        .unwrap();
