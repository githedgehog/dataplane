...
use crate::config::routing::bgp::BgpConfig;
use crate::config::routing::bgp::BgpNeighType;
use crate::config::routing::bgp::BgpNeighbor;
use crate::config::routing::bgp::BgpOptions;
use crate::config::routing::bgp::BgpUpdateSource;
use crate::config::routing::bgp::NeighSendCommunities;
use crate::config::routing::bgp::Redistribute;
use crate::config::routing::bgp::VrfImports;
use crate::config::routing::bgp::{AfIpv4Ucast, AfIpv6Ucast, AfL2vpnEvpn};
use crate::config::routing::bgp::{BgpNeighCapabilities, Protocol};
...
impl Render for AfIpv4Ucast {
    ...
    fn render(&self, bgp: &BgpConfig) -> ConfigBuilder {
        ...
        bgp.neighbors
            .iter()
            .filter_map(|neigh| neigh.ipv4_unicast.then_some(neigh))
...
impl Render for AfIpv6Ucast {
    ...
    fn render(&self, bgp: &BgpConfig) -> ConfigBuilder {
        ...
        bgp.neighbors
            .iter()
            .filter_map(|neigh| neigh.ipv6_unicast.then_some(neigh))
            .for_each(|neigh| cfg += format!(" neighbor {} activate", neigh.ntype));
...
impl Render for AfL2vpnEvpn {
    ...
    fn render(&self, bgp: &BgpConfig) -> ConfigBuilder {
        ...
        bgp.neighbors
            .iter()
            .filter_map(|neigh| neigh.l2vpn_evpn.then_some(neigh))
            .for_each(|neigh| cfg += format!(" neighbor {} activate", neigh.ntype));

        if self.adv_all_vni {
            cfg += format!(" advertise-all-vni");
        }
        if self.adv_default_gw {
            cfg += format!(" advertise-default-gw");
        }
        if self.adv_svi_ip {
            cfg += format!(" advertise-svi-ip");
        }
...
pub mod tests {
    ...
    fn test_bgp_render() {
        let mut bgp = BgpConfig::new(65000)
            .set_router_id(Ipv4Addr::from_str("7.0.0.100").expect("Bad address"));
        ...
        let imports = VrfImports::new()
            .set_routemap("Import-into-vrf-1")
            .add_vrf("VPC-2")
            .add_vrf("VPC-3")
            .add_vrf("VPC-4");
        ...
        /* configure ipv4 vrf imports */
        let imports = VrfImports::new()
            .set_routemap("Import-into-vrf-1-ipv6")
            .add_vrf("VPC-2")
            .add_vrf("VPC-3")
            .add_vrf("VPC-4");
