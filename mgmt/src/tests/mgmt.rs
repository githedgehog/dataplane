        ...
        bolero::check!()
            ...
            .for_each(|(mutation, bit, agent): (Mutation, bool, GatewayAgent)| {
                ...
                if let Ok(validated) = &outcome {
                    ...
                    // `OverlappingPrefixes` case below demonstrates.
                    assert!(
                        !bit,
...
///
/// It does **not** catch ambiguity the chain resolves at *run* time, and says so plainly. Delete the `OverlappingPrefixes` check from `VpcRouteTable::validate`, so that two peers
...
/// downstream consumer -- and the overlap rule is exactly that -- such
