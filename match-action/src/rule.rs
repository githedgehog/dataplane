...
impl<T: MaskBits> MaskSpec<T> {
    ...
    ///
    /// Prefer this to `new(value, all_ones)`, which makes the caller spell an all-ones bit pattern
    /// as a `T` -- and `NextHeader::new(0xff)` is not a protocol.
    #[must_use]
...
mod tests {
    ...
    /// The constructors are defined by what they match, not by the bits they store.
    /// Exhaustive over `u8`, so "nothing else" covers all 256 inputs rather than a sample.
    #[test]
