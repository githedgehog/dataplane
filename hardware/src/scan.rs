...
impl Node {
    ...
    /// ## Visibility
    ///
    /// This method deliberately (attemtps to) side step cgroup / isolcpu type restrictions regarding
...
mod test {
    use crate::{
        ...
        pci::{device::DeviceId, vendor::VendorId},
    };

    #[test]
    #[n_vm::in_vm]
    fn collect_them_em_all_and_bind_them() {
        // let mut hardware_file = std::fs::File::create("/tmp/hardware.yml").unwrap();
        let system = Node::scan_all();
        let nics: Vec<_> = system
            .iter()
            .filter_map(|node| match node.attributes() {
                Some(NodeAttributes::Pci(dev)) => {
                    if dev.vendor_id() == VendorId::new(0x1af4).unwrap()
                        && dev.device_id() == DeviceId::new(0x1041)
                    {
        ...
        let Some(mut nic) = system.iter().find_map(|node| match node.attributes() {
            Some(NodeAttributes::Pci(dev)) => {
                if dev.address() == target_pci_address
                    && dev.vendor_id() == VendorId::new(0x1af4).unwrap()
                    && dev.device_id() == DeviceId::new(0x1041)
                {
