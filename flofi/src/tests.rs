...
use crate::Flofi;
use crate::context::{FlofiContext, FlofiContextWriter};
...
fn make_flofi(ctx: FlofiContext) -> (Flofi, FlofiContextWriter) {
    let writer = FlofiContextWriter::default();
    writer.store(ctx);
    (Flofi::new("test-flofi", writer.get_reader()), writer)
}
...
// static-NAT prefix.
fn static_nat_plus_masquerade_context() -> FlofiContext {
    context(
        &[("vpc1", 100), ("vpc2", 200)],
        vec![peering(
            "vpc1-to-vpc2",
...
fn port_forwarding_destination_sets_flag_and_is_protocol_aware() {
    let (mut flofi, _) = make_flofi(dst_port_forwarding_context());
...
fn static_nat_destination_sets_static_dst_flag() {
    let (mut flofi, _) = make_flofi(dst_static_context());
    let out = run(
        &mut flofi,
        packet(
            Some(vpcd(100)),
...
fn ipv6_packet_through_the_nf() {
    let (mut flofi, _) = make_flofi(ipv6_context());
    let out = run(
        &mut flofi,
        packet(
            Some(vpcd(100)),
...
fn flow_key_attached_for_stateful_plus_static_nat_first_packet() {
    let (mut flofi, _) = make_flofi(static_nat_plus_masquerade_context());
...
fn context_writer_hot_swaps_routing() {
    let (mut flofi, writer) = make_flofi(source_nat_context());
