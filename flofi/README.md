...
Two independent actors touch flofi: the **control plane** publishes a routing context,
and the **data plane** reads it per packet. They are decoupled by an `ArcSwap` slot, so
config can be swapped atomically while packets are in flight.

```mermaid
flowchart LR
    subgraph CP["Control plane (mgmt)"]
        cfg["ValidatedOverlay<br/>(peering config)"]
        --> build["FlofiContext::try_from"]
        --> writer["FlofiContextWriter.store()"]
    end

    writer -. "atomic publish" .-> slot[("Slot / ArcSwap<br/>FlofiContext")]
    slot -. "load() per packet" .-> flofi

    subgraph DP["Data plane pipeline"]
        direction LR
        ingress --> iprouter1 --> icmp --> flowlookup["flow-lookup"]
        flowlookup --> flofi(["flofi"]) --> staticnat["static-NAT"]
        staticnat --> portfw["port-fwd"] --> masq["masquerade"]
        masq --> iprouter2 --> egress
    end

    style flofi fill:#2d6,stroke:#161,color:#000
```

Key point: flofi runs **after** `flow-lookup` (so a packet may already carry flow state)
and **before** the three NAT stages (which consume the flags flofi sets).
...
- `tcp` / `udp` tables: keyed on **prefix + port range** (`TwoTuple`).
- `other` table: keyed on **prefix only** (`Singleton`) -- used for ICMP etc.
- An expose with no protocol restriction (`L4Protocol::Any`) is fanned out into all three.

Two filters are applied while building:

- **local end** only includes exposes that `can_init_connection()` (i.e. not port-forwarding).
- **remote end** only includes exposes that `can_receive_connection()` (i.e. not masquerade).

That is why a masquerade destination is unreachable and a port-forwarding source cannot
initiate -- they are simply never inserted into the relevant table.

---

## 3. The lookup: two stages

This is the heart of the crate (`PeeringTables::lookup` / `VpcTable::lookup`). The mental
...
## 4. The per-packet path (`Flofi::process_packet`)

The lookup above is only reached for packets that need it. The NF first tries to **bypass**
the lookup using flow state left by `flow-lookup`, and it is responsible for **invalidating**
stale flows on a config change.

```mermaid
flowchart TD
    A["packet arrives at flofi"] --> B{"overlay?<br/>dst_vpcd unset?<br/>not done?"}
...
- **Invalidation authority.** flofi cannot fully validate stateful flows (it lacks NAT
  context), so it defers those to the NAT NFs. But it _is_ the authority for flows that need
  no state: after a config change it invalidates them (dst VPC changed, a NAT requirement
  appeared/disappeared, or the route now needs no state at all) so a fresh flow is created.
  See `should_invalidate_flow` for the exact branches.
...
| Term                 | Meaning                                                                                                            |
| -------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **VPC discriminant** | Identity of a VPC on the wire (a VNI). Source is known on ingress; destination is what flofi computes.             |
