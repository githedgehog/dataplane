...
///
/// The same guard the masquerade and static-NAT properties carry, and for the
/// same three reasons: instrumentation and emulation each buy a handful of cases,
/// and a corpus replay is deliberately one. `BOLERO_RANDOM_ITERATIONS=0` over a
/// saved entry runs exactly that entry, and a property that legitimately skips it
/// -- `nothing_is_forwarded_that_was_not_published` skips every published probe --
/// then reports `0 packets reached ... across 1 configurations` and fails. That is
/// precisely the run a developer makes to confirm a fix.
///
/// Two, not a fraction of a measured run: a floor calibrated on one machine stands
/// down on a slower one, and a guard that never judges is not a guard.
