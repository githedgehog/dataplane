...
//!
//! The third and last NAT flavour, and the one where the campaign's only confirmed configuration
//! bug lived -- `fix(config): Refuse a port-forwarding expose the dataplane cannot build`. That was
...
//! membership tests, as everywhere else in this campaign.
...
/// # Three gates, not one
///
/// Break testing this property turned up something worth writing down: **no single edit opens that
/// door.** A port past the published range is refused independently by
///
