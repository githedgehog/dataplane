...
// `FlowTable::new` takes a *shard* count, not a capacity -- capacity is set
// separately, by `set_capacity`.  Named `FLOW_CAPACITY` and set to 4096, this
// asked for 4096 DashMap shards per case and set no capacity whatsoever.  16 is
// what every other call site in the tree passes.
...
impl Fabric {
    pub(crate) fn build(exposes: &[VpcExpose]) -> Option<Self> {
        ...
        Some(Self {
            flow_table: Arc::new(FlowTable::new(FLOW_SHARDS)),
            writer,
            rules,
            peer,
        })
    ...
    /// Build a worker using this fabric's rules and the shared `flows` table.
    ///
    /// This models workers with different rule snapshots racing to create the same flow.
    ...
    /// Install `exposes`, migrate matching flows to `genid`, and invalidate the rest. Keep the
    /// fixture's original rule set for assertions about how the flows were opened.
    ...
    /// Whether `addr:port` belongs to the rule that publishes `published`.
    ///
    /// Checking the matched rule prevents accepting another service's backend. The generator
    /// currently gives rules disjoint address blocks; overlapping targets are needed to
    /// exercise that distinction.
    ///
    /// Check range membership here. `distinct_published_tuples_reach_distinct_targets` checks
    /// mapping uniqueness without repeating `PortFwEntry::map_address_port`'s arithmetic.
