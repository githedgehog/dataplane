...
mod tests {
    ...
    use crate::stateful::ippalloc::{NatDefaultAllocator, PoolTableKey};
    ...
    fn addr_v4(ip: &str) -> Ipv4Addr {
        Ipv4Addr::from_str(ip).unwrap()
    }
    fn addr_v4_bits(ip: &str) -> u32 {
        addr_v4(ip).to_bits()
    }
    ...
    fn build_allocator() -> Result<NatDefaultAllocator, ConfigError> {
    ...
    fn test_build_allocator() {
        ...
        */
        ...
        assert!(bitmap.contains_range(addr_v4_bits("10.2.0.0")..=addr_v4_bits("10.2.0.7")));
