    ...
    match (src_ip, ip) {
        ...
        (IpAddr::V6(src_ip), IpAddr::V6(allocated_ip)) => {
            match new_allocator.reserve_ipv6_port(proto, dst_vpcd, src_ip, allocated_ip, port) {
                ...
                Err(e) => {
                    error!("Failed to reserve {ip} {port_u16}: {e}. Invalidating flow...");
                    Err(())
                }
            }
        }
...
pub(crate) fn validate_stateful_nat_flows(
        ...
        .filter(|flow_info| flow_info.is_valid())
    {
        ...
        // We've found a peering with stateful NAT between the VPCs that this flow is exchaned.
        ...
        if nat_peering.peering.local.exposes.iter().any(|e| {
            ...
            if re_reserve_ip_and_port(new_allocator, flow_info.as_ref(), ip, port).is_err() {
                flow_info.invalidate_pair();
            } else {
