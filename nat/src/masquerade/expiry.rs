...
// Derived from the timeouts under test rather than written out.
// `Masquerade::TIMEOUT_SCALE` multiplies those by 100 under `emulated`, and a
// literal does not follow: a fixed 30s advance is six lifetimes natively and a
// fifteenth of one under qemu, so the flow is still live when the test asks
// whether it expired. Virtual time is free, so tracking the scale costs nothing.
const ONEWAY: u64 = crate::Masquerade::MASQUERADE_ONEWAY_TIMEOUT.as_secs();

/// Comfortably past the longest of the three masquerade timeouts.
const PAST_EXPIRY: Duration = Duration::from_secs(ONEWAY * 6);

/// Inside even the shortest of them, so a flow refreshed at this cadence stays up.
...
const LONG_SILENCE: Duration = Duration::from_mins(5);
