...
/// A v4 address pair for the fixtures. Every fixture source is unicast.
...
// Two VPCs reuse one private prefix but masquerade onto separate public ranges.
...
// Stop exposing VPC-1's original private source while retaining its public range.
...
// Remove masquerading while retaining the peering.
...
async fn test_full_config_unidirectional_nat_overlapping_destination() {
    ...
    let vpcd_context = FlowFilterContext::try_from(config.external().overlay()).unwrap();
    let vpcd_context_writer = FlowFilterContextWriter::new();
    vpcd_context_writer.store(vpcd_context);
    let mut vpcdlookup = FlowFilter::new("vpcd-lookup", vpcd_context_writer.get_reader());
...
// Walk the server-initiated graceful-close states.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
...
// Private addresses shared across VPCs must retain their VPC-specific translations.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
async fn test_masquerade_reconfig_two_vpcs_sharing_a_private_prefix() {
    ...
    // An identical config keeps the allocator and advances only the flow generation.
...
// Remove one peering without disturbing flows on another.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
async fn test_masquerade_reconfig_drops_a_flow_whose_peering_is_gone() {
    ...
    // VPC-1 loses its peering with VPC-3; VPC-2 keeps both.
    let mut vpc_table = VpcTable::new();
    let _ = vpc_table.add(Vpc::new("VPC-1", "AAAAA", 100).expect("Failed to add VPC"));
    let _ = vpc_table.add(Vpc::new("VPC-2", "BBBBB", 200).expect("Failed to add VPC"));
    let _ = vpc_table.add(Vpc::new("VPC-3", "CCCCC", 300).expect("Failed to add VPC"));
...
// Stop exposing one flow's private source while retaining its public range.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
...
// Send an ICMP Echo through the pipeline and hand back the whole output packet, so a caller can
// reach the flow the masquerade stage attached to it. The narrower `check_packet_icmp_echo_new`
// above projects out the four fields its callers compare and drops the metadata with them.
...
/// The ICMP Query session an ICMP Error message reports on must outlive the report.
///
/// A spoofable message must not be able to delete state. Masquerade releases the public address
/// and port along with the flow, so a NAT that honoured every Destination Unreachable it received
/// would let an off-path attacker both break a ping it cannot observe and churn the port pool.
///
/// Stated over packets rather than at the predicate, because the requirement is about a session
/// surviving, and only the pipeline -- allocator, flow table, ICMP handler -- has a session.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
async fn an_icmp_error_does_not_tear_down_the_query_session_it_reports_on() {
    ...
    // Host 1.1.2.3 pings 3.3.3.3. Masquerade allocates a public address and a translated
    // identifier for the Query session.
    ...
    // A second Echo Request over the same session, so that FlowLookup hands us the flow the
    // first one created.
    let echo = icmp_echo_through(
        &mut pipeline,
        vni(100),
    ...
    // A router on the far side answers with Destination Unreachable, embedding the Echo Request
...
/// The same Destination Unreachable, about a TCP flow, still tears that flow down.
///
/// The companion to the REQ-6 test above, and the reason it is not vacuous. REQ-6 is scoped to
/// ICMP Query payloads; the invalidation it forbids is an optimization we keep everywhere else,
/// and a "fix" that simply stopped invalidating would satisfy the requirement while silently
/// dropping the optimization. This is the assertion that says which of the two happened.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
async fn an_icmp_error_about_a_tcp_flow_still_tears_it_down() {
    let (flow_table, mut pipeline, _allocw) = test_setup(1, &build_overlay_2vpcs());

    // A lone SYN: the flow exists and is one-way, which is the state the optimization targets.
    let mut syn = tcp_packet_to_masquerade();
    syn.try_tcp_mut().unwrap().set_syn(true);
    ...
    // Ask again so FlowLookup hands us the flow the first packet created.
    let mut syn = tcp_packet_to_masquerade();
    syn.try_tcp_mut().unwrap().set_syn(true);
    let out = process_packet(&mut pipeline, syn);
    let flow = out.meta().flow_info.clone().expect("no flow for the SYN");
    ...
    // A router on the far side reports the SYN undeliverable.
    let (_, _, _, _, _, _, done_reason) = check_packet_icmp_error(
        &mut pipeline,
        vni(200),
        vni(100),
...
// The IPv6 counterparts of the v4 ICMP helpers above.
...
//# or delete the NAT Session that pertains to the embedded
//# payload within the ICMP Error packet.
/// The same as the IPv4 case, over `ICMPv6`.
///
/// A separate test rather than a parameter, because it is a separate arm of the predicate and the
/// citation interlock found it by noticing that nothing reached it. The IPv4 version of this test
/// held while the IPv6 arm could be deleted outright.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
...
/// `Packet Too Big` is IPv6's Path MTU Discovery, and must not cost the flow either.
///
/// The IPv6 counterpart of `path_mtu_discovery_does_not_tear_down_the_flow_that_triggered_it`. The
/// two families spell the recoverable case differently -- a Destination Unreachable *code* in v4, a
/// separate *type* in v6 -- which is why one test cannot cover both.
#[tokio::test]
#[cfg_attr(not(emulated), traced_test)]
