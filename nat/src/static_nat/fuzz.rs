...
/// rather than picked.
///
/// Measured natively 2026-09-06: 1060 to 2863 configurations per property here,
/// where the masquerade twin reaches 12 to 28. The floor matches the twin's
/// rather than this crate's own headroom, so the two do not drift apart, and it
/// is deliberately the smallest sample a rate can be computed from at all rather
/// than a fraction of either measurement -- a floor scaled to a fast machine
/// stands down on a slow one, which is the failure mode of a guard.
