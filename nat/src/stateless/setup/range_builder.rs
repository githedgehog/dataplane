...
fn add_offset_to_address_and_port(
    ...
    debug_assert!(port_range.start() <= port && port <= port_range.end());
    ...
    fn test_add_offset_to_address_with_ports() {
        assert_eq!(
            add_offset_to_address_and_port(
                &addr_v4("1.0.0.0"),
                4200,
                PortRange::new_checked(4000, 4999).unwrap(),
        ...
        assert_eq!(
            add_offset_to_address_and_port(
                &addr_v6("::"),
                0,
                PortRange::new_checked(0, u16::MAX).unwrap(),
    ...
    fn test_create_new_ranges() {
        ...
        assert_eq!(
            create_new_ranges(
                ...
                PortRange::new_checked(10, 12_000).unwrap(),
            ),
            vec![IpPortRange::new(
                IpRange::new(addr_v4("1.0.0.1"), addr_v4("1.0.0.1")),
                PortRange::new_checked(100, 10100).unwrap(),
            )]
        );
        ...
        assert_eq!(
            ...
            vec![
                IpPortRange::new(
                    IpRange::new(addr_v4("1.0.0.1"), addr_v4("1.0.0.1")),
                    PortRange::new_checked(4200, 4999).unwrap(),
                ),
                IpPortRange::new(
                    IpRange::new(addr_v4("1.0.0.2"), addr_v4("1.0.0.2")),
                    PortRange::new_checked(4000, 4800).unwrap(),
                )
            ]
        );
