...
impl Iterator for RangeBuilder<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut total_ranges_size = PrefixSize::U128(0);

        while total_ranges_size < orig_prefix_size {
            ...
            // Update state for next loop iteration, or next call

            total_ranges_size += range_size;
            // Do not update orig_offset_cursor if we're done processing the current prefix
            if total_ranges_size < orig_prefix_size {
