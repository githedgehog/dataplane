...
/// Model-checks interface configuration, runtime state, and vrf attachments.
#[cfg(test)]
mod iftable_properties {
    ...
    /// Non-default vrfs, all of which may be removed.
    ...
    fn apply(world: &mut World, model: &mut Model, change: &Change) {
        ...
        match change {
            ...
            Change::AddAddress { iface, address } => {
                ...
                // Unknown interfaces are logged and ignored by `absorb_first`.
    ...
    fn check(world: &World, model: &Model, at: &str) {
        ...
        for view in [
            ...
            for (index, ifindex) in ifaces.iter().enumerate() {
                ...
                // Attachments must not outlive their vrfs.
