...
impl Fib {
    ...
    /// The key this fib is filed under.
    ///
    /// # Panics
    ///
    /// Panics if the fib has no id. `FibWriter::new` always sets one and nothing clears it, so
    /// the only way here is `Fib::default()`, which exists for `left-right`'s second copy and is
    /// overwritten by the first publish. Dropping `FibKey::Unset` moved this panic from `as_u32`
    /// to here, which is closer to the cause but on a path a read guard can reach -- so the
    /// failure mode is a routing thread panicking rather than a line in the log. That is the
    /// intended trade: an unidentified fib in the table is a bug that should stop, not one that
    /// should be forwarded through.
    ///
    /// `Identity<FibKey> for Fib` fixes the return type, so this cannot become `Option<FibKey>`
    /// without changing that trait too.
    pub fn get_id(&self) -> FibKey {
        let Some(vrfid) = self.id else {
            unreachable!(
                "fib has no id: a Fib reached the table without FibWriter::new setting one"
            )
        };
        FibKey::Id(vrfid)
    }
