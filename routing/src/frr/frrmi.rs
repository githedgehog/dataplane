...
impl IoBuffer {
    ...
    /// Tell the length of the data in a message (encoded as |length|genid|data|).
    ///
    /// This is the value of the length field, so it counts |data| alone: the `HEADER_LEN`
    /// octets of |length|genid| are not included.
    ///
    /// If less than `HEADER_LEN` has been read it is not possible to know how big the message
    /// is yet.
    #[must_use]
