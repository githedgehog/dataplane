...
impl Vrf {
    pub fn add_route_rpc(&mut self, iproute: &IpRoute, vrf0: Option<&Vrf>, rstore: &RmacStore) {
        ...
        // `Vrf` substitutes a drop; warn here because only RPC distinguishes absent from rejected
        // next-hops.
        if nhops.is_empty() {
...
/// Checks RPC translation against expectations derived directly from wire fields.
#[cfg(test)]
mod rpc_properties {
    use super::*;
    ...
    /// Index of the intentionally invalid prefix.
    ...
    /// Independent oracle for route-origin conversion.
    ...
    /// Independent oracle for next-hop conversion and rejection.
    fn expected_key(spec: &NhopSpec, origin: RouteOrigin) -> Option<NhopKey> {
        ...
        // Encapsulated next-hops resolve through the underlay.
