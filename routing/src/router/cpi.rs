...
/// Properties over the control-plane operations, driven through a whole [`RoutingDb`].
///
/// This is the layer above `rpc_adapt`: it picks the vrf, decides what a lookup failure means, and
/// chains an operation's effects into the rest of the database -- an rmac arriving refreshes the
/// fibs of the vrfs on its vni, and deleting a route can delete the vrf with it. Almost none of it
/// was covered.
#[cfg(test)]
mod cpi_properties {
    use super::*;
    ...
    /// The vrf the overlay routes live in, and the vni it is reachable by.
    const OVERLAY_VRF: VrfId = 7;
    const OVERLAY_VNI: u32 = 3000;
    /// The interface the underlay route egresses on.
    ...
    /// Remote vtep addresses an overlay next-hop may point at. Both are covered by the underlay
    /// route installed below, so reachability is never the reason a route fails to resolve.
    fn vteps() -> Vec<IpAddr> {
    ...
    /// A database with an interface table, an underlay route in the default vrf towards the vteps,
    /// and one overlay vrf on [`OVERLAY_VNI`].
    fn fabric() -> RoutingDb {
        ...
        // the underlay: 7.0.0.0/8 out of an interface, so a vtep address resolves
        let vrf0 = db
            .vrftable
    ...
    /// An overlay route: prefix reachable by vxlan to `vtep` on [`OVERLAY_VNI`].
    ...
    /// The entries a vrf's fib offers for one prefix.
    ...
    /// Every entry is one the forwarder can execute, and any `Drop` in it comes before anything
    /// that would act on the packet.
    ///
    /// The second half is why an entry like `[Drop, Egress]` -- which resolution does produce, for
    /// an encapsulation that could not be completed -- is safe: the forwarder stops at the first
    /// instruction that finishes the packet, so a `Drop` reached first means nothing after it runs.
    ...
    fn the_pools_are_the_size_the_generator_thinks() {
        ...
        // every vtep must be reachable through the underlay route, so a failure to resolve is only
        // ever about the router mac
    ...
    /// An overlay route drops until the router mac for its vtep arrives, and encapsulates after.
    ///
    /// This is the evpn data path end to end: the route resolves through the underlay either way,
    /// but the encapsulation cannot be completed without a router mac, so until one arrives the
    /// only safe thing is to drop. When `Rmac::add` stores one it refreshes the fibs on that vni,
    /// which is what turns the entry into an encapsulate-then-egress.
    #[test]
    fn an_overlay_route_drops_until_its_router_mac_arrives() {
        bolero::check!().with_generator(Fabrics).cloned().for_each(
            |(vtep, mac): (usize, usize)| {
                ...
                // no router mac yet: the encapsulation cannot be completed
    ...
    /// Withdrawing a router mac does not stop traffic.
    ///
    /// `Rmac::del` marks the entry stale rather than removing it, and `VxlanEncapsulation::resolve`
    /// accepts a stale one -- "ok if we found a mac, even if the entry is stale". Forwarding to a
    /// mac that may have moved beats blackholing the vni while the control plane catches up.
    #[test]
    fn withdrawing_a_router_mac_leaves_the_route_forwarding() {
        bolero::check!().with_generator(Fabrics).cloned().for_each(
            |(vtep, mac): (usize, usize)| {
                ...
                // the fib is only rebuilt on the next refresh, so ask for one
    ...
    /// A route for a vrf we do not have fails on add, and succeeds on delete until we have a config.
    ///
    /// The asymmetry is deliberate: a delete for a vrf we never had is a route we do not have
    /// either, so reporting failure would leave frr retrying something already true. Once a config
    /// has been applied there is no such excuse, and the same lookup is a real failure.
    #[test]
    ...
    /// Deleting the last route of a vrf on its way out takes the vrf with it.
    #[test]
    fn deleting_the_last_route_of_a_dying_vrf_removes_it() {
        ...
        // while the vrf is active, deleting its routes leaves it in place
        ...
        // now the same delete empties it, which makes it deletable, which removes it
        assert_eq!(route.del(&mut db), RpcResultCode::Ok);
        assert!(
    ...
    /// An interface address is refused unless both the mask and the interface index are usable.
    #[test]
    fn an_interface_address_is_refused_unless_it_is_usable() {
        let cases = [
            // (ifindex, mask, expected)
