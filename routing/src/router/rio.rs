...
mod tests {
    ...
    /// How long a test waits on rio before deciding it is never going to answer.
    ///
    /// These are wall-clock waits on a real socket, not virtual ones, and every stage between the
    /// datagram and the fib gets much more expensive under coverage instrumentation -- a run that
    /// answers in well under a second clean can miss ten seconds instrumented, on a runner already
    /// carrying the rest of the matrix. Scaled with the build rather than raised outright, so an
    /// uninstrumented hang is still reported in ten seconds instead of two minutes.
