...
mod tests {
    ...
    // How long a test waits for rio to answer on the socket. Both arms are a
    // deadline for a hung subsystem, not a delay anything waits out, so they
    // are set well clear of the worst observed rather than near it: on a loaded
