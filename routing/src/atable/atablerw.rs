...
/// Model-checks pending writer state separately from the last state published to readers.
#[cfg(test)]
mod atable_properties {
    ...
    /// A table mutation and whether it publishes the resulting state.
    #[derive(Debug, Clone)]
    enum Change {
        Add {
            iface: usize,
    ...
    fn the_pools_are_the_size_the_generator_thinks() {
        ...
        // A replacement must be observable.
    ...
    fn a_clear_and_repopulate_is_invisible_until_published() {
        ...
        // Match the resolver's clear/repopulate/publish sequence.
        writer.clear(false);
