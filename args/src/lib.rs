...
///
/// Protections inude both basic linux DAC read only mode, as well as write, truncation, and extension sealing, as well
...
/// further mitigate opportunities for the data to be corrupted / mutatted.
...
impl MemFile {
    ...
    /// You should make sure no memory maps to this file are open when this finallize operation is invoked.
    ///
    /// # Panics
    ///
...
impl Display for NetworkDeviceDescription {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let NetworkDeviceDescription::Pci(addr) = self;
        write!(f, "{addr}")
    }
}
...
impl LaunchConfiguration {
    ...
    pub fn inherit() -> LaunchConfiguration {
        ...
        // The acttual objective here is to ensure all enums are valid and that all pointers point within the
        // archive.
            ...
            driver: match &value.driver {
                Some(driver) if driver == "dpdk" => {
                    let use_nics = value
                        ...
                        .iter()
                        .map(|x| {
                            PciAddress::try_from(x.as_str()).map(NetworkDeviceDescription::Pci)
                        })
                        .collect::<Result<Vec<_>, _>>()
                        .map_err(InvalidCmdArguments::InvalidPciAddress)?;
                    ...
                    .into_iter()
                    .chain(use_nics.iter().flat_map(|nic| match nic {
                        NetworkDeviceDescription::Pci(pci_address) => {
                            ["--allow".to_string(), format!("{pci_address}")]
                        }
                    }))
                    .collect();
                ...
                Some(other) => panic!("{other} is not a valid driver: must be kernel or dpdk"),
                None => panic!("no driver specified"),
            },
...
#[derive(Parser, serde::Deserialize, serde::Serialize)]
...
pub struct CmdArgs {
    ...
    #[arg(long, value_name = "PCI devices to probe (dpdk driver only)")]
    allow: Vec<String>,
