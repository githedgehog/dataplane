...
pub struct VpcPeeringStats {
    pub label: String, /* not needed, just for convenience */
    pub src: VpcDiscriminant,
    pub dst: VpcDiscriminant,
    pub src_vpc: String, /* may not use if use label */
    pub dst_vpc: String, /* may not use if use label */
...
impl PacketStats {
    ...
    fn update_vpcmatrix(
        ...
        if let Some(cell) = self.vpcmatrix.get_mut(&(sdisc, ddisc)) {
            ...
            // create a new cell
            let label = format!("{}-to-{}", &smap.name, &dmap.name);
            let mut cell = VpcPeeringStats::new(sdisc, ddisc, &smap.name, &dmap.name, label);
