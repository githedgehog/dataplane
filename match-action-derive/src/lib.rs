...
///
/// Every match field's type must implement `FixedSize` (how it is laid out in the key) *and*
/// `Display` (how it is shown to a human). `Display` is not optional on purpose: a table can only
/// report the rules it holds if every field can render itself, and a classifier an operator cannot
/// inspect is a classifier they cannot debug. A field type missing `Display` shows up as an
/// unsatisfied `MaskSpec<T>: Display`-style bound on the generated rule struct.
...
fn expand(input: &DeriveInput) -> syn::Result<TokenStream2> {
    ...
    // rule) and phantom fields. Phantom fields are excluded from the key layout
    // and never participate in matching (they exist only to carry type-level markers).
    ...
    for (i, field) in fields.iter().enumerate() {
        ...
        // the struct field itself -- or from its `#[cli(column_name = "...")]`, for fields whose
        // identifier is not what an operator should be shown -- so the label can never drift from
        // the value it labels.
...
///
/// This exists so a column heading lives next to the field it heads, rather than in a lookup table
/// in whichever crate happens to be rendering -- the same reason the label is taken from the struct
/// field in the first place, extended to fields whose identifier is not what an operator should be
/// shown (`src_vni` is a VNI, but the column means "which VPC did this come from").
