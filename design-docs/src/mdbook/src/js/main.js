...
const absToRelativeLinks = () => {
	const links = document.querySelectorAll('a[href^="/"]');
	links.forEach(link => {
		console.log("path to root", pathToRoot);
		console.log("link: ", link);
		const oldHref = link.getAttribute("href");
		const oldUrl = new URL(oldHref, window.location.href);
		if (oldUrl.search !== "") {
			console.warn("found unexpected search query on absolute URL, not rewriting")
			return;
		}
		const newUrl = new URL(pathToRoot + oldHref.slice(1), window.location.href);
		if (window.location.pathname === newUrl.pathname) {
			link.setAttribute("href", newUrl.hash);
		} else {
			link.setAttribute("href", pathToRoot + newUrl.pathname + newUrl.hash);
		}
	});
};
