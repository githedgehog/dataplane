...
use crate::parse::{
    DeParse, DeParseError, LengthError, Parse, ParseError, Reader, Step, StepWith, Writer,
};
use crate::tcp::Tcp;
use crate::udp::Udp;
use crate::vlan::Vlan;
...
impl DeParse for Net {
    ...
    fn write(&self, buf: &mut [u8]) -> Result<NonZero<usize>, DeParseError<Self::Error>> {
        match self {
            Net::Ipv4(ip) => ip.write(buf),
            Net::Ipv6(ip) => ip.write(buf),
        }
    }
}
...
impl DeParse for Transport {
    ...
    fn write(&self, buf: &mut [u8]) -> Result<NonZero<usize>, DeParseError<Self::Error>> {
        match self {
            ...
            Transport::Udp(x) => x.write(buf),
            Transport::Icmp4(x) => x.write(buf),
            Transport::Icmp6(x) => x.write(buf),
        }
    }
}
        ...
        match self {
            ...
            IpV6Ext(ext) => {
                if let Ipv6(ipv6) = self {
                    ext.step_with(&ipv6.next_header(), cursor).map(Header::from)
                } else {
...
impl DeParse for Packet {
    ...
    fn size(&self) -> NonZero<usize> {
        ...
        let eth_size = self.eth.size().get();
        let vlan_size = self.vlan.iter().map(|v| v.size().get()).sum::<usize>();
        ...
        NonZero::new(eth_size + vlan_size + net_size + transport_size)
