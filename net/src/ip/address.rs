...
mod narrowing_rejects_non_unicast {
    ...
    /// `TryFrom<IpAddr>` used to build the newtype directly, so it accepted anything of the
    /// right family. A narrowed multicast address then tripped an `unreachable!` further down
    /// the packet path rather than being refused at the boundary.
    #[test]
