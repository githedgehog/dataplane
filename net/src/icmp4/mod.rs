...
impl Icmp4 {
    ...
    ///
    /// Parameter Problem is on the `ICMPv4` list and off the `ICMPv6` one; see
    /// [`crate::icmp6::Icmp6::supports_extensions`] for why.
    ...
    pub(crate) fn parse_payload(&self, cursor: &mut Reader) -> Option<EmbeddedHeaders> {
        ...
        let (mut headers, consumed) =
            EmbeddedHeaders::parse_with(EmbeddedIpVersion::Ipv4, &cursor.inner[embedded_start..])
                .ok()?;
...
mod test {
    ...
    //# The ICMP Extension Structure MUST NOT be appended to any of the other
    //# ICMP messages mentioned in Section 4.
    /// The `ICMPv4` types that may carry an extension structure, and the ones that may not.
    ///
    /// The counterpart of `only_two_icmpv6_types_can_carry_an_extension_structure`, and the
    /// citation needs both: RFC 4884 gives one list per family and they differ, so a test of
    /// either one alone says nothing about the other. That is how Parameter Problem came to be on
    /// the IPv6 list.
    #[test]
    ...
    /// The length attribute counts 32-bit words for `ICMPv4`, and lives at octet 5.
    ///
    /// The `ICMPv6` counterpart uses octet 4 and 64-bit words; see
    /// `the_icmpv6_length_attribute_counts_64_bit_words`.
    #[test]
