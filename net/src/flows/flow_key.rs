...
impl FlowAddrs {
    /// # Errors
    ///
    /// Returns [`FlowKeyError::NotUnicast`] if the destination address is not unicast.
...
mod tests {
    ...
    /// A packet to a non-unicast destination gets no flow key at all.
    ///
    /// This is the one behaviour `FlowAddrs` changed rather than merely made unrepresentable,
    /// and no generator can reach it any more: the old key drew a unicast destination and
    /// widened it, and the new one cannot hold a non-unicast destination to begin with. So the
    /// case has to be written out. Previously such a packet got a key whose `reverse` could
    /// never match -- a flow entry nothing would ever look up again. Every caller of
    /// `FlowKey::try_from` already handled the error: the packet passes untagged rather than
    /// being dropped.
    #[test]
    fn a_packet_to_a_multicast_destination_has_no_flow_key() {
        for (src, dst) in [("1.2.3.4", "224.0.0.5"), ("1.2.3.4", "255.255.255.255")] {
            let mut ip = crate::ipv4::Ipv4::default();
            ip.set_source(
                UnicastIpv4Addr::new(src.parse().unwrap_or_else(|_| unreachable!()))
                    .unwrap_or_else(|_| unreachable!()),
            );
            ip.set_destination(dst.parse().unwrap_or_else(|_| unreachable!()));
            let net = Net::Ipv4(ip);
            let err = FlowAddrs::from_net(&net)
                .expect_err("a non-unicast destination must not produce a flow key");
            assert!(
                matches!(err, FlowKeyError::NotUnicast(got) if got.to_string() == dst),
                "expected NotUnicast({dst}), got {err}"
            );
        }
    }
