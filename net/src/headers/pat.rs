...
impl<'a> EmbeddedMatcherMut<'a, EmbeddedStart, (), ()> {
    ...
    ///
    /// Used by [`EmbeddedHeadersView::look_mut`](super::embedded_view::EmbeddedHeadersView)
...
// `EmbeddedHeadersView::look_mut` for shapes whose final inner layer is
...
mod opt_properties {
    ...
    /// Check that every optional matcher accepts all strict matches on read and mutable paths.
    ///
    /// This covers unconditional, cursor-advancing, and present/absent/wrong-variant families.
    ...
    /// Require both strict matches and cases accepted only by the optional form.
    ...
    /// Apply the same invariant to independently generated embedded matcher families.
    ...
    // Embedded matchers expose neither optional enum method, and only the mutable form exposes the
    // transport enum, so those pairs cannot be expressed here.
    embedded_opt_is_weaker!(
    ...
    /// Check that three-way matchers accept matching or absent layers and reject wrong variants.
    #[test]
    ...
    /// Check that cursor-based optional matchers advance only when they consume a layer.
    #[test]
    ...
    /// Check that an optional extension still enters the strict extension-gap region.
    #[test]
    ...
    /// The embedded combinators observe only the inner accumulator.
    ...
    /// Pin embedded-combinator semantics: combinators observe the inner accumulator, while `done()`
    /// requires both the outer and inner matches.
    #[test]
    ...
    /// Check the transport enum's strict gap rejection on the mutable embedded matcher.
    #[test]
