...
mod contract {
    ...
    /// Draws header layouts with varied VLAN and extension-header stacks.
    ///
    /// Unlike [`CommonHeaders`], this generator exercises structural view and matcher behavior.
    /// Wire-level `next_header` consistency is intentionally outside its contract.
    ...
    ///
    /// A quarter of v6 runs follow RFC 8200's recommended order so multi-extension shapes occur
    /// often enough to exercise while retaining randomly ordered cases.
    fn ext_run<D: Driver>(
        driver: &mut D,
        v4: bool,
    ...
    /// Draws an ICMP error whose quote contains exactly one selected extension header.
    ///
    /// Use this for properties that inspect successful matches; use [`ShapedIcmpError`] when misses
    /// are also part of the property.
    #[allow(dead_code)] // constructed through `.with_generator()`
    ...
    /// Draws ICMP errors with varied outer and quoted-packet structure.
    ///
    /// This varies VLAN and extension stacks, quote presence, address-family agreement, and
    /// truncated transport headers. It is structural: extension `next_header` fields are not kept
    /// wire-consistent because the matchers under test do not read them.
    #[allow(dead_code)] // constructed through `.with_generator()`
    #[repr(transparent)]
