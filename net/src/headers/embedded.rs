...
impl DeParse for EmbeddedHeaders {
    ...
    fn size(&self) -> NonZero<u16> {
        let net_ext = if self.net.is_some() {
            self.net_ext_headers_len()
        } else {
            debug_assert!(
        ...
        NonZero::new(self.net_headers_len() + net_ext + self.transport_headers_len())
...
mod tests {
    ...
    // Compare address-delta folding with a fresh checksum over a complete quoted packet. The
    // starting checksum must be valid for an RFC 1624 update to be meaningful.
    mod address_folding {
        use super::*;
