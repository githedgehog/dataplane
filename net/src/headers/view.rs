...
/// `HeadersView<T>` sound.  Each reproduces a would-be foot-gun and
/// documents, via the compiler, that the path is closed.
///
/// ## No owning constructor
///
/// Private fields prevent forging a `HeadersView<T>` from an arbitrary
/// `Headers` (E0423).  The only entry points
/// ([`Headers::as_view`] / [`Headers::as_view_mut`]) run
...
/// let _w: HeadersView<(&Eth,)> = HeadersView(h, std::marker::PhantomData);
/// ```
///
/// ## `HeadersView<T>` is not [`Clone`]
///
/// Cloning through a `&HeadersView<T>` would manufacture an owned
/// shape-proven value that outlives its borrow of `Headers`
/// (E0277: `HeadersView<T>: Clone` unsatisfied).
///
/// ```compile_fail,E0277
/// use dataplane_net::eth::Eth;
/// use dataplane_net::headers::{Headers, HeadersView};
...
/// let _owned: HeadersView<(&Eth,)> = clone_it(w);
/// ```
///
/// ## No deref to [`Headers`]
///
/// `HeadersView<T>` does not implement [`Deref`](std::ops::Deref) or
/// [`DerefMut`](std::ops::DerefMut) with
/// `Target = Headers`.  If it did, a caller holding
/// `&mut HeadersView<T>` could reshape the underlying data while the
/// `PhantomData<T>` proof stayed intact (E0599: no method found --
/// `HeadersView` does not autoderef to `Headers`).
///
/// ```compile_fail,E0599
/// use dataplane_net::eth::Eth;
/// use dataplane_net::headers::Headers;
/// let mut h = Headers::default();
/// let w = h.as_view_mut::<(&Eth,)>().unwrap();
/// let _ = w.eth_mut();
/// ```
///
...
/// `HeadersView`, so it cannot be called twice concurrently (E0499:
...
/// ```
///
/// ## `ViewStep` is crate-private
///
/// External crates cannot implement `ViewStep`, so they cannot
/// add new per-layer extraction logic and thereby forge a
/// `HeadersView<T>` that sidesteps `matches` (E0603: trait is private).
///
/// ```compile_fail,E0603
/// use dataplane_net::headers::view::ViewStep;
/// ```
///
/// ## `ViewStepMut` is crate-private
///
/// Same seal on the mutable side (E0603).
///
/// ```compile_fail,E0603
/// use dataplane_net::headers::view::ViewStepMut;
/// ```
///
/// ## `Shape` is sealed
///
...
impl<T> HeadersView<T> {
    /// Crate-private accessor used by [`EmbeddedHeadersView`](super::embedded_view::EmbeddedHeadersView)
    ...
    /// [`Look::look`] / [`LookMut::look_mut`] / [`Headers::as_view`].
    #[inline]
...
// ===========================================================================
// ViewStepMut: mutable per-layer dispatch onto MatcherMut
// ===========================================================================
//
...
// bookkeeping, `ViewStepMut::chain` extends a `MatcherMut` chain by
...
///
/// Mirrors [`ViewStep`] but dispatches through
...
pub(crate) trait ViewStepMut<Pos>: Sized {
...
impl<Pos> ViewStepMut<Pos> for Vlan
where
    Vlan: Within<Pos>,
{
    #[inline(always)]
...
macro_rules! impl_net_step_mut {
    ($T:ty, $method:ident) => {
        impl<Pos> ViewStepMut<Pos> for $T
        where
            $T: Within<Pos>,
        {
            #[inline(always)]
...
impl<Pos> ViewStepMut<Pos> for Vxlan
where
    Vxlan: Within<Pos>,
{
    #[inline(always)]
...
macro_rules! impl_view_arity_1 {
    ($A:ident) => {
        ...
        impl<'x, $A> LookMut<(&'x $A,)> for HeadersView<(&'x $A,)>
        where
            $A: ViewStep<()> + ViewStepMut<()>,
            Self: 'x,
        {
            type RefsMut<'a>
                = (&'a mut $A,)
            ...
            {
                unsafe {
                    // SAFETY: HeadersView invariant: matches(&self.0) was true.
                    // (see notes at top of file)
                    core::hint::assert_unchecked(<(&'x $A,) as sealed::Sealed>::matches(&self.0));
                }
                let m = self.0.pat_mut();
                let m = <$A as ViewStepMut<()>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
...
macro_rules! impl_view_arity_2 {
    ($A:ident, $B:ident) => {
        ...
        impl<'x, $A, $B> LookMut<(&'x $A, &'x $B)> for HeadersView<(&'x $A, &'x $B)>
        where
            $A: ViewStep<()> + ViewStepMut<()>,
            $B: ViewStep<$A> + ViewStepMut<$A>,
            Self: 'x,
        {
            type RefsMut<'a>
                = (&'a mut $A, &'a mut $B)
                ...
                Self: 'a,
            {
                ...
                let m = self.0.pat_mut();
                let m = <$A as ViewStepMut<()>>::chain(m);
                let m = <$B as ViewStepMut<$A>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
...
macro_rules! impl_view_arity_3 {
    ($A:ident, $B:ident, $C:ident) => {
        ...
        impl<'x, $A, $B, $C> LookMut<(&'x $A, &'x $B, &'x $C)>
            for HeadersView<(&'x $A, &'x $B, &'x $C)>
        where
            $A: ViewStep<()> + ViewStepMut<()>,
            $B: ViewStep<$A> + ViewStepMut<$A>,
            $C: ViewStep<$B> + ViewStepMut<$B>,
            Self: 'x,
        {
            type RefsMut<'a>
                ...
                Self: 'a,
            {
                ...
                let m = <$A as ViewStepMut<()>>::chain(m);
                let m = <$B as ViewStepMut<$A>>::chain(m);
                let m = <$C as ViewStepMut<$B>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
    ...
    ($A:ident, $B:ident, $C:ident, $D:ident) => {
            ...
            Self: 'x,
        {
                ...
                Self: 'a,
            {
                ...
                let m = <$B as ViewStepMut<$A>>::chain(m);
                let m = <$C as ViewStepMut<$B>>::chain(m);
                let m = <$D as ViewStepMut<$C>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
    ...
    ($A:ident, $B:ident, $C:ident, $D:ident, $E:ident) => {
            ...
            Self: 'x,
        {
                ...
                Self: 'a,
            {
                ...
                let m = <$C as ViewStepMut<$B>>::chain(m);
                let m = <$D as ViewStepMut<$C>>::chain(m);
                let m = <$E as ViewStepMut<$D>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
            ...
            Self: 'x,
        {
            ...
            fn look_mut<'a>(&'a mut self) -> Self::RefsMut<'a>
                                ...
                                Self: 'a,
                            {
                                ...
                                let m = <$D as ViewStepMut<$C>>::chain(m);
                                let m = <$E as ViewStepMut<$D>>::chain(m);
                                let m = <$F as ViewStepMut<$E>>::chain(m);
                                match m.done() {
                                    Some(t) => t,
                                    // SAFETY: HeadersView invariant guarantees the chain hits.
                                    // (see notes at top of file)
    ...
    ($A:ident, $B:ident, $C:ident, $D:ident, $E:ident, $F:ident, $G:ident) => {
            ...
            Self: 'x,
        {
                ...
                Self: 'a,
            {
                ...
                let m = <$E as ViewStepMut<$D>>::chain(m);
                let m = <$F as ViewStepMut<$E>>::chain(m);
                let m = <$G as ViewStepMut<$F>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
    ...
    ($A:ident, $B:ident, $C:ident, $D:ident, $E:ident, $F:ident, $G:ident, $H:ident) => {
            ...
            Self: 'x,
        {
                ...
                Self: 'a,
            {
                ...
                let m = <$F as ViewStepMut<$E>>::chain(m);
                let m = <$G as ViewStepMut<$F>>::chain(m);
                let m = <$H as ViewStepMut<$G>>::chain(m);
                match m.done() {
                    Some(t) => t,
                    // SAFETY: HeadersView invariant guarantees the chain hits.
                    // (see notes at top of file)
...
mod tests {
    ...
    //
    // `look_mut` dispatches through `MatcherMut`, not `ViewStep::step`.
            ...
            .for_each(|(a, b, c)| {
                for src in [a, b, c] {
                    ...
                    let imm: Option<(*const Eth, *const Vlan, *const Ipv4, *const Tcp)> =
                        headers.as_view::<(&Eth, &Vlan, &Ipv4, &Tcp)>().map(|w| {
            ...
            .for_each(|(a, b, c)| {
                for src in [a, b, c] {
                    ...
                    let mutp: Option<(*const Eth, *const Ipv6, *const HopByHop, *const Tcp)> =
                        headers
                            .as_view_mut::<(&Eth, &Ipv6, &HopByHop, &Tcp)>()
                            .map(|w| {
...
/// Differential checks for [`HeadersView`]'s unsafe extraction boundary.
///
/// [`sealed::Sealed::matches`] licenses [`Look::look`] to use `unwrap_unchecked`. These tests compare
/// that decision with the safe [`Matcher`](super::pat::Matcher) before extraction, then compare layer
/// addresses so equal-valued duplicate layers remain distinguishable.
#[cfg(test)]
mod view_properties {
    use crate::eth::Eth;
    ...
    /// The same for a shape that must consume exactly one VLAN tag.
    #[test]
...
mod view_mut_properties {
    ...
    /// Map each matcher method to the layer type used to derive its view shape.
    ...
    /// Return layer addresses without naming the tuple arity.
    ///
    /// Addresses distinguish equal-valued duplicate layers and expose cursor-selection errors.
    trait Addrs {
    ...
    /// Check acceptance and selected-layer identity at every supported arity.
    ///
    /// Acceptance protects the unchecked extraction; address comparison checks the separately
    /// generated cursor traversal.
    ...
    // Cover every supported arity, including first-step rejection.
    ...
    // Exact extension shapes exercise strict gap checks and extension-cursor threading. The mutable
    // path also compares the `Headers` and pre-split `Fields` implementations.
    arity_agrees!(
        read_ext_v6_one,
        mutable_ext_v6_one,
        SometimesHeadless,
    ...
    // VXLAN is outside the linear header stack and bypasses both gap checks, so cover it explicitly.
    arity_agrees!(
        read_vxlan_v4,
        mutable_vxlan_v4,
        ShapedHeaders,
