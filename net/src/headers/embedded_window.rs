...
/// Today only [`Window<T>`] implements this trait; same future-proofing
/// rationale as [`HasHeaders`].
pub(crate) trait HasHeadersMut: HasHeaders {
    fn as_headers_mut(&mut self) -> &mut Headers;
}

impl<T> HasHeadersMut for Window<T> {
    #[inline(always)]
    fn as_headers_mut(&mut self) -> &mut Headers {
        Window::as_headers_mut(self)
    }
}
...
/// unchecked under the `EmbeddedWindow<W, U>` shape invariant.
...
macro_rules! impl_embedded_arity_1 {
    ($A:ident) => {
        ...
        impl<'x, W, $A> EmbeddedLookMut<(&'x $A,)> for EmbeddedWindow<W, (&'x $A,)>
        where
            W: HasHeadersMut,
            ...
            Self: 'x,
        {
                ...
                Self: 'a,
            {
                ...
                match m.done() {
                    Some(((a,),)) => (a,),
                    // SAFETY: EmbeddedWindow invariant guarantees the chain hits.
...
macro_rules! impl_as_embedded_arity_1 {
    ($A:ident) => {
            ...
            EmbeddedHeaders: Within<$A>,
        {
            ...
            #[inline]
            #[must_use]
            pub fn as_embedded_mut<U>(&mut self) -> Option<&mut EmbeddedWindow<Self, U>>
            where
                U: EmbeddedShape,
            {
                let matched = self
                ...
                let p = std::ptr::from_mut(self).cast::<EmbeddedWindow<Self, U>>();
...
mod tests {
    ...
    fn look_mut_agrees_with_look_for_v4_tcp() {
        ...
        let mutp: (*const Ipv4, *const TruncatedTcp) = {
            let outer: &mut Window<OuterIcmp4> = h.as_window_mut().unwrap();
    ...
    fn look_mut_mutation_propagates() {
        ...
        {
            let outer = h.as_window_mut::<OuterIcmp4>().unwrap();
            let ew = outer.as_embedded_mut::<InnerV4Tcp>().unwrap();
        ...
        let outer = h.as_window::<OuterIcmp4>().unwrap();
        let ew = outer.as_embedded::<InnerV4Tcp>().unwrap();
        let (_, tcp) = ew.look();
        match tcp {
    ...
    fn outer_mut_round_trip_preserves_pointer_identity() {
        ...
        let ew = outer.as_embedded_mut::<InnerV4Tcp>().unwrap();
        let outer_back: &mut Window<OuterIcmp4> = ew.outer_mut();
        assert!(std::ptr::eq(
