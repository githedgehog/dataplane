...
impl Parse for Vxlan {
    ...
    fn parse<T: AsRef<[u8]>>(buf: T) -> Result<(Self, NonZero<u16>), ParseError<Self::Error>> {
        let buf = buf.as_ref();
        if buf.len() < Vxlan::MIN_LENGTH.cast().get() {
            return Err(ParseError::Length(LengthError {
                expected: Vxlan::MIN_LENGTH.cast(),
                actual: buf.len(),
            }));
        }
        let slice = &buf[..Vxlan::MIN_LENGTH.cast().get()];
    ...
    fn parse_noise() {
        bolero::check!()
            ...
            .for_each(|slice: &[u8; MIN_LENGTH_USIZE]| {
                ...
                assert_eq!(
                    &write_back_buffer[4..bytes_written.cast().get()],
                    &slice[4..bytes_written.cast().get()]
                );
            });
    }
        ...
        bolero::check!().with_type().for_each(|vni: &Vxlan| {
            ...
            match vni.deparse(&mut too_small_buffer) {
                Err(DeParseError::Length(e)) => {
                    assert_eq!(e.expected, Vxlan::MIN_LENGTH.cast());
