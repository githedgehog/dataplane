...
mod size_budget {
    ...
    /// The budget allows for the `test_meta` field, because a workspace build unifies features
    /// and turns that feature on: asserting the shipping size unconditionally fails under
    /// `cargo nextest run --workspace` while passing for `-p dataplane-net`, which is a
    /// property of the build rather than of the struct.
    #[test]
    fn meta_stays_small() {
        // `test_meta` adds a cold `Option<Box<TestMeta>>`, which is 8 bytes and ships only to
        // tests. The shipping budget is the `else` arm; both arms are exact today.
        const BUDGET: usize = if cfg!(feature = "test_meta") { 80 } else { 72 };
        assert!(
