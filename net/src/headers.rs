...
use crate::parse::{
    DeParse, DeParseError, IllegalBufferLength, LengthError, NonZeroUnSizedNumericUpcast, Parse,
    ParseError, ParsePayload, ParsePayloadWith, Reader, Writer,
};
use crate::tcp::Tcp;
...
impl DeParse for Net {
    ...
    fn deparse<T: AsMut<[u8]>>(&self, buf: T) -> Result<NonZero<u16>, DeParseError<Self::Error>> {
        match self {
...
impl DeParse for Headers {
    ...
    fn size(&self) -> NonZero<u16> {
        ...
        let vlan: u16 = self.vlan.iter().map(|v| v.size().get()).sum();
...
mod contract {
    ...
    impl TypeGenerator for Headers {
        ...
        fn generate<D: Driver>(driver: &mut D) -> Option<Self> {
            ...
            let mut arbitrary_bytes: [u8; 2 * size_of::<Headers>()] = driver.gen()?;
            let arbitrary_eth: Eth = driver.gen()?;
    ...
    impl ValueGenerator for CommonHeaders {
        ...
        fn generate<D: Driver>(&self, driver: &mut D) -> Option<Self::Output> {
            ...
            match common_eth_type {
                ...
                CommonEthType::Ipv6 => {
                    let common_next_header: ipv6::CommonNextHeader = driver.gen()?;
                    let ipv6 =
...
mod test {
    ...
    use crate::parse::{DeParse, DeParseError, NonZeroUnSizedNumericUpcast, Parse, ParseError};

    fn parse_back_test(headers: &Headers) {
        let mut buffer = [0_u8; 1024];
        let bytes_written = match headers.deparse(&mut buffer[..headers.size().cast().get()]) {
            Ok(written) => written,
        ...
        let (parsed, bytes_parsed) = match Headers::parse(&buffer[..bytes_written.cast().get()]) {
            Ok(k) => k,
