...
//! ASM probe for `Window<T>::look`.
//!
//! Build and dump assembly:
//!
//! ```text
//! cargo rustc --release --example window_asm_probe --features test_buffer \
//!     -- --emit=asm -C debuginfo=0
//! # Output lands in target/release/examples/window_asm_probe-<hash>.s
//! ```
//!
...
//! probe into `main`).  Inside the probe the Window API calls **do**
...
use dataplane_net::headers::{Headers, Look, Window};
...
pub fn probe_v4_tcp_matches_only(h: &Headers) -> bool {
    h.as_window::<(&Eth, &Ipv4, &Tcp)>().is_some()
}

// look() on an already-proven Window: should compile to pure offset
// arithmetic with NO branches (the shape invariant was proved at
// `as_window` time, so no runtime check should remain).
#[inline(never)]
#[unsafe(no_mangle)]
pub fn probe_existing_window<'a>(
    w: &'a Window<(&Eth, &Ipv4, &Tcp)>,
) -> (&'a Eth, &'a Ipv4, &'a Tcp) {
    w.look()
}
