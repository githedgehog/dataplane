...
# Released hhfab tags, not `v0-master-<rev>` builds: the latter are pruned from ghcr and take the
# whole vlab down with them when they go.  The pin is frozen, so this moves only when someone bumps
# it deliberately.
declare fabricator_version
fabricator_version="$(jq --exit-status --raw-output '.pins.fabricator.version' "${SOURCE_DIR}/../../npins/sources.json")"
declare -r fabricator_version

docker exec vlab /bin/bash -c \
    "curl -fsSL 'https://i.hhdev.io/hhfab' | USE_SUDO=false INSTALL_DIR=. VERSION=${fabricator_version} bash"
