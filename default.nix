...
let
  ...
  # Exactly the `RUSTFLAGS` the selected sanitizers contribute, by difference.
  #
  # Subtraction rather than a substring test because there is no substring that
  # catches them all: `-Clink-arg=-static-libasan` shares none with
  # `-Zsanitizer=address` or `-Zexternal-clangrt`, and each sanitizer spells its
  # runtime differently.  Doing it by difference also means a flag added to
  # `profiles.nix` later is handled without anyone remembering this exists.
  sanitizer-rustflags = lib.subtractLists profile-unsanitized'.RUSTFLAGS profile'.RUSTFLAGS;
  ...
  n-preinit-static = workspace."n-preinit".overrideAttrs (orig: {
    env = orig.env // {
      ...
      #
      # The sanitizer comes back out.  rustc refuses the combination outright
      # -- "sanitizer is incompatible with statically linked libc" -- so a
      # sanitized build otherwise cannot get as far as producing a guest
      # image, and `just sanitize=address test` dies in `setup-roots` rather
      # than in anything it was trying to measure.  Dropping it here rather
      # than dropping in-VM testing from sanitized builds is the right trade:
      # the pre-init is the scaffolding that execs the code under test, not
      # the code under test.  It costs a rebuild of two small crates, its
      # only dependencies being `n-vm-protocol` and a feature-trimmed `nix`.
  ...
  #
  # Not subject to `closure-check`, which examines `dataplane.tar` alone. This
  # image is a developer's toolbox and is never shipped, so a compiler or a
  # source tree in it is not a leak.
  #
